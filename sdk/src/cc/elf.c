...
void put_got_entry(CCState *s1, int reloc_type, unsigned long size, int info, int sym_index)
{
    ...
    if (s1->dynsym)
    {
        ...
        if (reloc_type == R_386_JMP_SLOT)
        {
            ...
            if (s1->output_type == CC_OUTPUT_EXE)
...
int cc_load_object_file(CCState *s1, int fd, unsigned long file_offset)
{
    ...
    found:
        ...
        if (sh->sh_addralign > (Elf32_Word)s->sh_addralign)
            s->sh_addralign = sh->sh_addralign;
...
int cc_load_archive(CCState *s1, int fd)
{
    ...
    for (;;)
    {
        ...
        if (!strcmp(ar_name, "/"))
        {
            // COFF symbol table
            if (s1->alacarte_link)
                return cc_load_alacarte(s1, fd, size);
        }
        ...
        {
            if (cc_load_object_file(s1, fd, file_offset) < 0)
                return -1;
        }
