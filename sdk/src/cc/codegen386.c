...
void g(int c)
{
    if (ind == code_size)
    {
        ...
        if (code_size == 0)
            code_size = 1024;
        code = cc_realloc(code, code_size);
    }
    code[ind++] = c;
}
...
int gbranch(int type)
{
    ...
    if (br == branch_size)
    {
        ...
        if (branch_size == 0)
            branch_size = 128;
        branch = cc_realloc(branch, branch_size * sizeof(Branch));
    }
...
{
    if (branch[l].sym)
        error("internal error: symbol already assigned to label");
    branch[l].sym = sym;
}
...
void cut_code_buffer(CodeBuffer *cb)
{
    ...
    cb->code = (unsigned char *)cc_malloc(code_buffer_size);
    memcpy(cb->code, code + cb->ind, code_buffer_size);
    cb->branch = (Branch *)cc_malloc(branch_buffer_size * sizeof(Branch));
...
void gcode(void)
{
    ...
    while (more)
    {
        ...
        for (i = 0; i < br; ++i)
        {
            ...
            // Find next non-nop
            n = i + 1;
            while (branch[n].type == CodeNop || branch[n].type == CodeLine)
                n++;
            bn = branch + n;
            if (b->ind != bn->ind)
                continue;
    ...
    if (errs)
        error("internal error: code generation");
}
...
void gen_cvt_ftoi(int t)
{
    ...
    o(0x2dd9); // ldcw xxx
    greloc(external_global_sym(TOK___cc_int_fpu_control, &ushort_type, VT_LVAL), 0, 0);
    ...
    o(0x24);
    o(0x2dd9); // ldcw xxx
    greloc(external_global_sym(TOK___cc_fpu_control, &ushort_type, VT_LVAL), 0, 0);
