...
typedef struct CCState
{
    ...
    // Output format, see CC_OUTPUT_FORMAT_xxx
    int output_format;
...
Section *new_section(CCState *s1, const char *name, int sh_type, int sh_flags);
...
Section *find_section(CCState *s1, const char *name);
...
// asm.c
int cc_assemble(CCState *s1, int do_preprocess);

void asm_instr(void);

void masm_instr(CCState *s1);

void asm_global_instr(void);

int find_constraint(ASMOperand *operands, int nb_operands, const char *name, const char **pp);

int asm_int_expr(CCState *s1);

void asm_expr(CCState *s1, ExprValue *pe);

void asm_expr_logic(CCState *s1, ExprValue *pe);

static void *resolve_sym(CCState *s1, const char *symbol, int type) { return NULL; }
...
int pe_load_def_file(struct CCState *s1, int fd);

int pe_test_res_file(void *v, int size);

int pe_load_res_file(struct CCState *s1, int fd);

// util.c
void *cc_malloc(unsigned long size);

void *cc_mallocz(unsigned long size);

void *cc_realloc(void *ptr, unsigned long size);

char *cc_strdup(const char *str);

void cc_free(void *ptr);

char *cc_basename(const char *name);

char *cc_fileextension(const char *p);
