...
char *cc_fileextension(const char *name)
{
    char *b = cc_basename(name);
    char *e = strrchr(b, '.');
...
void dynarray_add(void ***ptab, int *nb_ptr, void *data)
{
    ...
    if ((nb & (nb - 1)) == 0)
    {
        ...
        pp = cc_realloc(pp, nb_alloc * sizeof(void *));
        if (!pp)
            error("memory full");
        *ptab = pp;
    }
...
void cstr_realloc(CString *cstr, int new_size)
{
    ...
    while (size < new_size)
        size = size * 2;
    data = cc_realloc(cstr->data_allocated, size);
    if (!data)
        error("memory full");
...
void error_ex(CCState *s1, int is_warning, const char *fmt, va_list ap)
{
    char buf[2048];
    BufferedFile **f;
...
void init_util(void)
{
    ...
    // Init isidnum table
    for (i = 0; i < 256; i++)
        isidnum_table[i] = is_id(i) || is_num(i);
}
