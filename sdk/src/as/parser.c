/*
 * parser.c   source line parser for the assembler
 */
...
static int prefix_slot(enum prefixes prefix)
{
    ...
    case P_ASP:
        return PPS_ASIZE;
    default:
        as_error(ERR_PANIC, "Invalid value %d passed to prefix_slot()", prefix);
        return -1;
    }
}
...
static void process_size_override(insn *result, int operand)
{
    ...
    else
    {
        ...
        case S_QWORD:
            ...
            break;
        default:
            as_error(ERR_NONFATAL, "invalid size specification in"
                                   " effective address");
            break;
        }
    }
...
restart_parse:
    ...
    if (i != TOKEN_ID && i != TOKEN_INSN && i != TOKEN_PREFIX &&
        (i != TOKEN_REG || (REG_SREG & ~as_reg_flags[tokval.t_integer])))
    {
        as_error(ERR_NONFATAL, "label or instruction expected"
                               " at start of line");
    ...
    if (i == TOKEN_ID || (insn_is_label && i == TOKEN_INSN))
    {
        ...
        {
            as_error(ERR_WARNING | ERR_WARN_OL | ERR_PASS1,
                     "label alone on a line without a colon might be in error");
        }
        if (i != TOKEN_INSN || tokval.t_integer != I_EQU)
        {
            /*
             * TODO: location->segment could be NO_SEG, in which case
           ...
           (i == TOKEN_REG && !(REG_SREG & ~as_reg_flags[tokval.t_integer])))
    {
        ...
        if (i == TOKEN_PREFIX && tokval.t_integer == P_TIMES)
        {
            ...
            i = stdscan(NULL, &tokval);
            value =
                evaluate(stdscan, NULL, &tokval, NULL, pass0, as_error, NULL);
            i = tokval.t_type;
            if (!value)
            ...
            else
            {
                ...
                {
                    as_error(ERR_NONFATAL, "TIMES value %" PRId64 " is negative",
                             value->value);
                    result->times = 0;
                }
            }
        ...
        else
        {
            ...
            if (result->prefixes[slot])
            {
                if (result->prefixes[slot] == tokval.t_integer)
                    as_error(ERR_WARNING | ERR_PASS1,
    ...
    if (i != TOKEN_INSN)
    {
        ...
        {
            as_error(ERR_NONFATAL, "parser: instruction expected");
            result->opcode = I_none;
            return result;
        }
    }
        ...
        result->opcode == I_DY || result->opcode == I_INCBIN)
    {
        ...
        while (1)
        {
            ...
            first = false;
            fixptr = tail;
            eop = *tail = as_malloc(sizeof(extop));
            tail = &eop->next;
            eop->next = NULL;
            ...
            else if (i == TOKEN_STRFUNC)
            {
                ...
                else
                {
                    ...
                    {
                        as_error(ERR_NONFATAL, "invalid string for transform");
                        eop->type = EOT_NOTHING;
                    }
                }
                if (parens && i && i != ')')
                {
                    ...
                    if (i != ')')
                    {
                        as_error(ERR_NONFATAL, "unterminated %s function",
                                 funcname);
                    }
                }
                if (i && i != ',')
            ...
            if (i != ',')
            {
                as_error(ERR_NONFATAL, "comma expected after operand %d",
                         oper_num);
                result->opcode = I_none; /* unrecoverable parse error: */
                return result;           /* ignore this instruction */
            }
        ...
        if (result->opcode == I_INCBIN)
        {
            ...
            if (!result->eops || result->eops->type != EOT_DB_STRING)
                as_error(ERR_NONFATAL, "`incbin' expects a file name");
            else if (result->eops->next &&
                     result->eops->next->type != EOT_DB_NUMBER)
                as_error(ERR_NONFATAL, "`incbin': second parameter is"
                                       " non-numeric");
            else if (result->eops->next && result->eops->next->next &&
                     result->eops->next->next->type != EOT_DB_NUMBER)
                as_error(ERR_NONFATAL, "`incbin': third parameter is"
                                       " non-numeric");
            else if (result->eops->next && result->eops->next->next &&
    ...
    for (operand = 0; operand < MAX_OPERANDS; operand++)
    {
        ...
        { /* size specifiers */
            ...
            case S_SHORT:
                ...
                break;
            default:
                as_error(ERR_NONFATAL, "invalid operand size specification");
            }
            i = stdscan(NULL, &tokval);
        }
        ...
        {
            as_error(ERR_NONFATAL, "invalid use of FAR operand specifier");
        }

        value = evaluate(stdscan, NULL, &tokval,
                         &result->oprs[operand].opflags,
                         critical, as_error, &hints);
        i = tokval.t_type;
        ...
        { /* it was seg:offset */
            ...
            if (value[1].type != 0 || value->value != 1 ||
                REG_SREG & ~as_reg_flags[value->type])
                as_error(ERR_NONFATAL, "invalid segment override");
            ...
            else
            {
                ...
                if (!(REG_FSGS & ~as_reg_flags[value->type]))
        ...
        { /* find ] at the end */
            if (i != ']')
            {
                as_error(ERR_NONFATAL, "parser: expecting ]");
                recover = true;
            }
            else
            { /* we got the required ] */
                ...
                if (i != 0 && i != ',')
                {
                    as_error(ERR_NONFATAL, "comma or end of line expected");
                    recover = true;
                }
            }
        }
        ...
        { /* immediate operand */
            if (i != 0 && i != ',' && i != ':')
            {
                as_error(ERR_NONFATAL, "comma, colon or end of line expected");
                recover = true;
            }
            else if (i == ':')
            {
        ...
        { /* it's a memory reference */
            ...
            if (e->type != 0)
            { /* there'd better be nothing left! */
                as_error(ERR_NONFATAL,
        ...
        { /* it's not a memory reference */
            ...
            { /* it's a register */
                ...
                {
                    as_error(ERR_NONFATAL, "invalid operand type");
                    result->opcode = I_none;
                    return result;
                }
                ...
                result->oprs[operand].type |= as_reg_flags[value->type];
