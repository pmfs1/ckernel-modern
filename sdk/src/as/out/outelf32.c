/*
 * outelf32.c   output routines for the assembler to produce
...
/*
 * Special AS section numbers which are used to define ELF special
...
static int elf_make_section(char *name, int type, int flags, int align)
{
    ...
    if (!strcmp(name, ".text"))
        ...
        s->index = seg_alloc();
    add_sectname("", name);
    s->name = as_malloc(1 + strlen(name));
    strcpy(s->name, name);
    s->type = type;
    s->flags = flags;
    s->align = align;
    ...
    if (nsects >= sectlen)
        sects = as_realloc(sects, (sectlen += SECT_DELTA) * sizeof(*sects));
    sects[nsects++] = s;
...
static int32_t elf_section_names(char *name, int pass, int *bits)
{
    ...
    if (!strcmp(name, ".shstrtab") ||
        ...
        !strcmp(name, ".strtab"))
    {
        as_error(ERR_NONFATAL, "attempt to redefine reserved section"
                               "name `%s'",
                 name);
        return NO_SEG;
    }
    ...
    {
        if ((type && sects[i]->type != type) || (align && sects[i]->align != align) || (flags_and && ((sects[i]->flags & flags_and) != flags_or)))
            as_error(ERR_WARNING, "incompatible section attributes ignored on"
                         ...
                         int is_global, char *special)
{
    ...
    {
        /*
         * This is a AS special symbol. We never allow it into
    ...
    if (is_global == 3)
    {
        ...
        for (s = &fwds; *s; s = &(*s)->nextfwd)
            if (!strcmp((*s)->name, name))
            {
                ...
                expr *e;
                char *p = as_skip_spaces(as_skip_word(special));
    ...
    else
    {
        ...
        if (segment == def_seg)
        {
            ...
            if (segment != elf_section_names(".text", 2, &tempint))
                as_error(ERR_PANIC, "strange segment conditions in ELF driver");
        }
        for (i = 0; i < nsects; i++)
        {
    ...
    if (is_global == 2)
    {
        ...
        if (special)
        {
            ...
            if (err)
                as_error(ERR_NONFATAL, "alignment constraint `%s' is not a"
                                       " valid number",
                         special);
    ...
    if (sym->type == SYM_GLOBAL)
    {
        ...
        else if (sym->section != SHN_ABS)
        {
            ...
            if (special)
            {
                ...
                if (!as_strnicmp(special, "function", n))
                    sym->type |= STT_FUNC;
                else if (!as_strnicmp(special, "data", n) ||
                         !as_strnicmp(special, "object", n))
                    sym->type |= STT_OBJECT;
                else if (!as_strnicmp(special, "notype", n))
                    sym->type |= STT_NOTYPE;
                else
                    as_error(ERR_NONFATAL, "unrecognised symbol type `%.*s'",
                             n, special);
                special += n;

                special = as_skip_spaces(special);
                if (*special)
                {
                    n = strcspn(special, " \t");
                    if (!as_strnicmp(special, "default", n))
                        sym->other = STV_DEFAULT;
                    else if (!as_strnicmp(special, "internal", n))
                        sym->other = STV_INTERNAL;
                    else if (!as_strnicmp(special, "hidden", n))
                        sym->other = STV_HIDDEN;
                    else if (!as_strnicmp(special, "protected", n))
                ...
                if (*special)
                {
                    ...
                    while (special[n] && as_isspace(special[n]))
                        n++;
                    /*
                    ...
                    tokval.t_type = TOKEN_INVALID;
                    e = evaluate(stdscan, NULL, &tokval, &fwd, 0, as_error,
                                 NULL);
                    if (fwd)
                    {
                        sym->nextfwd = fwds;
    ...
    if (special && !special_used)
        as_error(ERR_NONFATAL, "no special symbol features supported here");
}
                                  ...
                                  int type, bool exact)
{
    ...
    {
        if (exact && offset != 0)
            as_error(ERR_NONFATAL, "unable to find a suitable global symbol"
                                   " for this reference");
        else
                    ...
                    int32_t segment, int32_t wrt)
{
    ...
    case OUT_ADDRESS:
    {
        ...
        if (segment != NO_SEG)
        {
            if (segment % 2)
            {
                as_error(ERR_NONFATAL, "ELF format does not support"
            ...
            else
            {
                ...
                {
                    as_error(ERR_NONFATAL, "ELF format cannot produce non-PC-"
                                           "relative PLT references");
                }
                else
                {
                    as_error(ERR_NONFATAL, "ELF format does not support this"
                                           " use of WRT");
        ...
        if (gnu16)
        {
            as_error(ERR_WARNING | ERR_WARN_GNUELF,
                     "8- or 16-bit relocations in ELF32 is a GNU extension");
        }
        else if (size != 4 && segment != NO_SEG)
        {
            as_error(ERR_NONFATAL, "Unsupported non-32-bit ELF relocation");
        }
        ...
        else
        {
            ...
            else if (wrt == elf_plt_sect + 1)
            {
                elf_add_reloc(s, segment, R_386_PLT32);
                     ...
                     wrt == elf_got_sect + 1)
            {
                as_error(ERR_NONFATAL, "ELF format cannot produce PC-"
...
static void elf_write(void)
{
    ...
    elf_foffs += align;
    elf_nsect = 0;
    elf_sects = as_malloc(sizeof(*elf_sects) * nsections);
    ...
    for (i = 0; i < nsects; i++)
    {
        elf_section_header(p - shstrtab, sects[i]->type, sects[i]->flags,
                           (sects[i]->type == SHT_PROGBITS ? sects[i]->data : NULL), true,
...
static int elf_directive(enum directives directive, char *value, int pass)
{
    ...
    if (directive == D_OSABI)
    {
        ...
        if (err)
        {
            as_error(ERR_NONFATAL, "`osabi' directive requires a parameter");
            return 1;
        }
        if (n < 0 || n > 255)
        {
            as_error(ERR_NONFATAL, "valid osabi numbers are 0 to 255");
            return 1;
        }
        elf_osabi = n;
        elf_abiver = 0;
        ...
        if (err || n < 0 || n > 255)
        {
            as_error(ERR_NONFATAL, "invalid ABI version number (valid: 0 to 255)");
            return 1;
        }
                            ...
                            int32_t segto)
{
    ...
    if (!stabs_filename)
    {
        stabs_filename = (char *)as_malloc(strlen(filename) + 1);
    ...
    else
    {
        if (strcmp(stabs_filename, filename))
        {
            ...
            /* why not as_free(stabs_filename); we're done with the old one */
...
static void stabs32_output(int type, void *param)
{
    ...
    if (type == TY_STABSSYMLIN)
    {
        if (debug_immcall)
        {
            ...
            numlinestabs++;
            el = (struct linelist *)as_malloc(sizeof(struct linelist));
...
static void stabs32_generate(void)
{
    ...
    allfiles = (char **)as_malloc(numlinestabs * sizeof(char *));
    ...
    strsize = 1;
    fileidx = (int *)as_malloc(numfiles * sizeof(int));
    for (i = 0; i < numfiles; i++)
    {
        fileidx[i] = strsize;
     ...
     */
    sbuf = (uint8_t *)as_malloc((numlinestabs * 2 + 4) *
                                sizeof(struct stabentry));
    ssbuf = (uint8_t *)as_malloc(strsize);
    rbuf = (uint8_t *)as_malloc(numlinestabs * 8 * (2 + 3));
    rptr = rbuf;
...
static void dwarf32_generate(void)
{
    ...
    arangeslen = saalen + 4;
    arangesbuf = pbuf = as_malloc(arangeslen);
    ...
    arangesrelbuf = pbuf = as_malloc(arangesrellen);
    ...
    pubnamesbuf = pbuf = as_malloc(pubnameslen);
    WRITELONG(pbuf, saalen); /* initial length */
    ...
    saa_wbytes(pinfo, as_signature, strlen(as_signature) + 1);
    ...
    infolen = saalen + 4;
    infobuf = pbuf = as_malloc(infolen);
    WRITELONG(pbuf, saalen); /* initial length */
    ...
    inforelbuf = pbuf = as_malloc(inforellen);
    ...
    abbrevbuf = pbuf = as_malloc(saalen);
    ...
    linebuf = pbuf = as_malloc(linelen);
    ...
    linerelbuf = pbuf = as_malloc(linerellen);
    saa_rnbytes(plinesrel, pbuf, saalen);
    saa_free(plinesrel);

    /* build frame section */
    framelen = 4;
    framebuf = pbuf = as_malloc(framelen);
    WRITELONG(pbuf, framelen - 4); /* initial length */

    /* build loc section */
    loclen = 16;
    locbuf = pbuf = as_malloc(loclen);
...
static void dwarf32_findfile(const char *fname)
{
    ...
    dwarf_clist = (struct linelist *)as_malloc(sizeof(struct linelist));
    dwarf_numfiles++;
    dwarf_clist->line = dwarf_numfiles;
    dwarf_clist->filename = as_malloc(strlen(fname) + 1);
...
static void dwarf32_findsect(const int index)
{
    ...
    dwarf_csect = (struct sectlist *)as_malloc(sizeof(struct sectlist));
    dwarf_nsections++;
