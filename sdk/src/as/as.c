/*
 * The assembler main program module
 */
 ...
 static void as_verror_gnu(int severity, const char *fmt, va_list args);
  ...
  * to output. It's used when someone explicitly requests that AS
 ...
 static void process_respfile(FILE *rfile)
 {
     ...
     buffer = as_malloc(ARG_BUF_DELTA);
     ...
     { /* Loop to handle all lines in file */
         ...
         *(p = &buffer[strcspn(buffer, "\r\n\032")]) = '\0';
         ...
         while (p > buffer && as_isspace(p[-1]))
             *--p = '\0';
 
         p = as_skip_spaces(buffer);
 ...
 static void parse_cmdline(int argc, char **argv)
 {
     ...
     *inname = *outname = *listname = *errname = '\0';
     ...
     /*
      * First, process the ASENV environment variable.
      */
 ...
 static void assemble_file(char *fname, StrList **depend_ptr)
 {
     ...
     if (cmd_sb == 32 && cmd_cpu < IF_386)
         as_error(ERR_FATAL, "command line: "
     ...
     for (passn = 1; pass0 <= 2; passn++)
     {
         ...
         preproc->reset(fname, pass1, &aslist,
 ...
 static void as_verror_gnu(int severity, const char *fmt, va_list ap)
 {
     ...
     as_verror_common(severity, fmt, ap);
 }
 ...
 static uint32_t get_cpu(char *value)
 {
     ...
     if (!strcmp(value, "486"))
         return IF_486;
     if (!strcmp(value, "586") || !as_stricmp(value, "pentium"))
         return IF_PENT;
     if (!strcmp(value, "686") ||
         !as_stricmp(value, "ppro") ||
         !as_stricmp(value, "pentiumpro") || !as_stricmp(value, "p2"))
         return IF_P6;
     ...
     if (!as_stricmp(value, "prescott"))
         return IF_PRESCOTT;
     if (!as_stricmp(value, "x64") ||
         !as_stricmp(value, "x86-64"))
         return IF_X86_64;
     if (!as_stricmp(value, "ia64") ||
         !as_stricmp(value, "ia-64") ||
         !as_stricmp(value, "itanium") ||
         !as_stricmp(value, "itanic") || !as_stricmp(value, "merced"))
         return IF_IA64;
