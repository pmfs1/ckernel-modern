...
static int do_directive(Token *tline) {
    ...
    switch (i) {
        ...
        case PP_EXITMACRO:
             ...
             */
            list_for_each(l, istk->expansion)if (l->finishes && l->finishes->name)
                    break;
...
static Token *expand_smacro(Token *tline) {
    ...
    while (tline) {             /* main token loop */
        ...
        if ((mname = tline->text)) {
             ...
             * necessary.
             */
            list_for_each(m, head)if (!mstrcmp(m->name, mname, m->casesense))
                    break;
            if (m) {
                mstart = tline;
                params = NULL;
...
static MMacro *is_mmacro(Token *tline, Token ***params_array) {
     ...
     */
    list_for_each(m, head)if (!mstrcmp(m->name, tline->text, m->casesense))
            break;
    if (!m)
        return NULL;
