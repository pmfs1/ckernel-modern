/*
 * preproc.c   macro preprocessor for the assembler
 */
 ...
 * used in as.h for the "real" condition codes. At _this_ level,
 ...
 * conditional compilation directives. Using the AS pre-processor does
 * not work, so we look for them specifically from the following list and
 * then jam in the equivalent AS directive into the input stream.
 */
...
/*
 * The standard macro set: defined in macros.c in the array as_stdmac.
...
#define skip_white_(x)                  \
    if (tok_type_((x), TOK_WHITESPACE)) \
    (x) = (x)->next
#define tok_is_(x, v) (tok_type_((x), TOK_OTHER) && !strcmp((x)->text, (v)))
#define tok_isnt_(x, v) ((x) && ((x)->type != TOK_OTHER || strcmp((x)->text, (v))))

/*
 * as_unquote with error if the string contains NUL characters.
 * If the string contains NUL characters, issue an error and return
 * the C len, i.e. truncate at the NUL.
 */
static size_t as_unquote_cstr(char *qstr, enum preproc_token directive)
{
    size_t len = as_unquote(qstr, NULL);
 ...
 * be nice to be able to use the AS pre-processor to do it).
 */
static char *check_tasm_directive(char *line)
{
    ...
    if (len)
    {
        ...
        {
            k = (j + i) / 2;
            m = as_stricmp(p, tasm_directives[k]);
            if (m == 0)
            {
                /* We have found a directive, so jam a % in front of it
                 * so that AS will then recognise it as one if it's own.
                 */
                p[len] = oldchar;
                len = strlen(p);
                oldline = line;
                line = as_malloc(len + 2);
                line[0] = '%';
                if (k == TM_IFDIFI)
                {
                    /*
                     * AS does not recognise IFDIFI, so we convert
                     * it to %if 0. This is not used in AS
 ...
 * flags') into AS preprocessor line number indications (`%line
 * lineno file').
 */
...
static void free_mmacro_table(struct hash_table *mmt)
{
    ...
    {
        as_free((void *)key);
        list_for_each_safe(m, tmp, m) free_mmacro(m);
    }
    hash_free(mmt);
}
...
hash_findi_add(struct hash_table *hash, const char *str)
{
    ...
    strx = as_strdup(str); /* Use a more efficient allocator here? */
...
static char *read_line(void)
{
    ...
    while (1)
    {
        ...
        if (p - buffer > bufsize - 10)
        {
            ...
            bufsize += BUF_DELTA;
            buffer = as_realloc(buffer, bufsize);
        ...
        else if (isnumstart(*p))
        {
            ...
            for (;;)
            {
                ...
                else if (c2 == '.')
                {
                    ...
                    if (as_isdigit(*r) || (is_hex && as_isxdigit(*r)) ||
                        (!is_hex && (*r == 'e' || *r == 'E')) ||
                        (*r == 'p' || *r == 'P'))
                    {
                        p = r;
...
static void *new_Block(size_t size)
{
    ...
    b->chunk = as_malloc(size);

    /* now allocate a new block for the next request */
    b->next = as_malloc(sizeof(Blocks));
...
static int ppscan(void *private_data, struct tokenval *tokval)
{
    ...
    if (tline->type == TOK_ID)
    {
        ...
        *s = '\0';
        ...
        return as_token_hash(ourcopy, tokval);
    }
    ...
    if (tline->type == TOK_STRING)
    {
        ...
        bq = tline->text[0];
        tokval->t_charptr = tline->text;
        tokval->t_inttwo = as_unquote(tline->text, &ep);
 ...
 * simple wrapper which calls either strcmp or as_stricmp
 * depending on the value of the `casesense' parameter.
 */
static int mstrcmp(const char *p, const char *q, bool casesense)
{
    return casesense ? strcmp(p, q) : as_stricmp(p, q);
}
...
{
    return casesense ? memcmp(p, q, l) : as_memicmp(p, q, l);
}
            ...
            if (!mstrcmp(m->name, name, m->casesense))
            {
                ...
                if (!once)
                {
                    error(ERR_WARNING, "context-local macro expansion"
                                       ...
                                       " AS 2.10, please see the AS Manual for"
                                       " more information");
                    once = 1;
                }
    ...
    *params = NULL;
    while (t)
    {
        ...
        if (*nparam + 1 >= paramsize)
        {
            ...
            *params = as_realloc(*params, sizeof(**params) * paramsize);
        }
        skip_white_(t);
        brace = false;
        if (tok_is_(t, "{"))
...
static bool if_condition(Token *tline, enum preproc_token ct)
{
    ...
    case PPC_IFCTX:
        ...
        while (true)
        {
            ...
            if (cstk && cstk->name && !as_stricmp(tline->text, cstk->name))
                j = true;
            tline = tline->next;
        }
        break;
    ...
    case PPC_IFIDNI:
        ...
        while ((t->type != TOK_OTHER || strcmp(t->text, ",")) && tt)
        {
            ...
            {
                size_t l1 = as_unquote(t->text, NULL);
                size_t l2 = as_unquote(tt->text, NULL);
    ...
    case PPC_IFMACRO:
    {
        ...
        searching.name = as_strdup(tline->text);
        ...
        while (mmac)
        {
            if (!strcmp(mmac->name, searching.name) &&
                (mmac->nparam_min <= searching.nparam_max || searching.plus) && (searching.nparam_min <= mmac->nparam_max || mmac->plus))
            {
                found = true;
                break;
            }
                          ...
                          int nparam, Token *expansion)
{
    ...
    else
    {
        ...
        smac = as_malloc(sizeof(SMacro));
        smac->next = *smhead;
        *smhead = smac;
    }
    smac->name = as_strdup(mname);
...
static bool parse_mmacro_spec(Token *tline, MMacro *def, const char *directive)
{
    ...
    if (tline && tok_type_(tline->next, TOK_ID) &&
        !as_stricmp(tline->next->text, ".nolist"))
    {
        tline = tline->next;
        def->nolist = true;
    }
...
static int do_directive(Token *tline)
{
    ...
    /*
     * TODO: We zap execution of PP_RMACRO, PP_IRMACRO, PP_EXITMACRO
    ...
    case PP_STACKSIZE:
        /* Directive to tell AS what the default stack size is. The
        ...
        if (as_stricmp(tline->text, "flat") == 0)
        {
        ...
        else if (as_stricmp(tline->text, "flat64") == 0)
        {
        ...
        else if (as_stricmp(tline->text, "large") == 0)
        {
        ...
        else if (as_stricmp(tline->text, "small") == 0)
        {
            /* All subsequent ARG directives are for a 16-bit stack,
    ...
    case PP_LOCAL:
         ...
         *
         * The '= LocalSize' at the end is ignored by AS, but is
    ...
    case PP_DEPEND:
        ...
        {
            StrList *sl = as_malloc(strlen(p) + 1 + sizeof sl->next);
            sl->next = NULL;
            strcpy(sl->str, p);
            *deptail = sl;
            deptail = &sl->next;
    ...
    case PP_INCLUDE:
        ...
        {
            inc->fname = src_set_fname(as_strdup(p));
    ...
    case PP_USE:
    {
        ...
        if (tline->type == TOK_STRING)
            as_unquote_cstr(tline->text, i);
        use_pkg = as_stdmac_find_package(tline->text);
        if (!use_pkg)
    ...
    case PP_POP:
        ...
        else
        {
            ...
            {
                if (p && (!cstk->name || as_stricmp(p, cstk->name)))
    ...
    issue_error:
    {
        ...
        if (tok_type_(tline, TOK_STRING) && !t)
        {
            ...
            p = tline->text;
            as_unquote(p, NULL); /* Ignore NUL character truncation */
            error(severity, "%s", p);
        }
        else
        {
    ...
    CASE_PP_IF:
        ...
        else
        {
            ...
            j = j < 0 ? COND_NEVER : j ? COND_IF_TRUE
                                       : COND_IF_FALSE;
        }
        cond = as_malloc(sizeof(Cond));
    ...
    case PP_IMACRO:
        ...
        defining = as_malloc(sizeof(MMacro));
        defining->max_depth =
        ...
        while (mmac)
        {
            if (!strcmp(mmac->name, defining->name) &&
                (mmac->nparam_min <= defining->nparam_max || defining->plus) && (defining->nparam_min <= mmac->nparam_max || mmac->plus))
            {
                error(ERR_WARNING | ERR_PASS1,
    ...
    case PP_EXITMACRO:
         ...
         */
        list_for_each(l, istk->expansion) if (l->finishes && l->finishes->name) break;
    ...
    case PP_REP:
        ...
        if (tok_type_(tline, TOK_ID) &&
            as_stricmp(tline->text, ".nolist") == 0)
        {
            nolist = true;
            do
            {
    ...
    case PP_IDEFINE:
        ...
        while (t)
        {
            if (t->type == TOK_ID)
            {
                list_for_each(tt, param_start) if (tt->type >= TOK_SMAC_PARAM &&
    ...
    case PP_IDEFSTR:
        ...
        macro_start->text = as_quote(p, strlen(p));
    ...
    case PP_STRLEN:
        ...
        macro_start = as_malloc(sizeof(*macro_start));
        macro_start->next = NULL;
        make_tok_num(macro_start, as_unquote(t->text, NULL));
        macro_start->a.mac = NULL;
    ...
    case PP_STRCAT:
        ...
        list_for_each(t, tline)
        {
            ...
            case TOK_WHITESPACE:
                break;
            case TOK_STRING:
                len += t->a.len = as_unquote(t->text, NULL);
                break;
            case TOK_OTHER:
        ...
        macro_start->text = as_quote(pp, len);
        as_free(pp);
        define_smacro(ctx, mname, casesense, 0, macro_start);
        free_tlist(tline);
        free_tlist(origline);
    ...
    case PP_LINE:
        ...
        if (tline)
        {
            as_free(src_set_fname(detoken(tline, false)));
        }
        free_tlist(origline);
        return DIRECTIVE_FOUND;
...
static int find_cc(Token *t)
{
    ...
    {
        k = (j + i) / 2;
        m = as_stricmp(t->text, conditions[k]);
        if (m == 0)
        {
            i = k;
            j = -2;
            ...
            else
            {
                ...
                if (!pos)
                {
                    ...
                    case '0':
                        ...
                        text = as_strdup(tmpbuf);
                        break;
                    case '%':
                        type = TOK_ID;
                        snprintf(tmpbuf, sizeof(tmpbuf), "..@%" PRIu64 ".",
                                 mac->unique);
                        text = as_strcat(tmpbuf, t->text + 2);
                        break;
                    case '-':
                        n = atoi(t->text + 2) - 1;
                        if (n >= mac->nparam)
                        ...
                        else
                        {
                            ...
                            if (inverse_ccs[cc] == -1)
                            {
                                ...
                                text = as_strdup(conditions[inverse_ccs[cc]]);
                        }
                        break;
                    case '+':
                        n = atoi(t->text + 2) - 1;
...
again:
    ...
    { /* main token loop */
        ...
        if ((mname = tline->text))
        {
             ...
             * necessary.
             */
            list_for_each(m, head) if (!mstrcmp(m->name, mname, m->casesense)) break;
            if (m)
            {
                mstart = tline;
                params = NULL;
                ...
                if (m->nparam == 0)
                {
                    ...
                    if (!m->expansion)
                    {
                        if (!strcmp("__FILE__", m->name))
                        {
                            ...
                            char *file = NULL;
                            src_get(&num, &file);
                            tline->text = as_quote(file, strlen(file));
                            tline->type = TOK_STRING;
                            as_free(file);
                            continue;
                        }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        nparam = 0;
                        sparam = PARAM_DELTA;
                        params = as_malloc(sparam * sizeof(Token *));
                        params[0] = tline->next;
                        paramsize = as_malloc(sparam * sizeof(int));
                        paramsize[0] = 0;
                        while (true)
                        { /* parameter loop */
                            /*
                            ...
                            if (tline->type == TOK_OTHER && tline->text[1] == 0)
                            {
                                ...
                                if (ch == ',' && !paren && brackets <= 0)
                                {
                                    ...
                                    {
                                        sparam += PARAM_DELTA;
                                        params = as_realloc(params,
                                                            sparam * sizeof(Token *));
                                        paramsize = as_realloc(paramsize,
                                                               sparam * sizeof(int));
                                    }
                                    params[nparam] = tline->next;
                                    paramsize[nparam] = 0;
...
static MMacro *is_mmacro(Token *tline, Token ***params_array)
{
     ...
     */
    list_for_each(m, head) if (!mstrcmp(m->name, tline->text, m->casesense)) break;
    if (!m)
        return NULL;
    ...
    while (m)
    {
        if (m->nparam_min <= nparam && (m->plus || nparam <= m->nparam_max))
        {
            ...
            { /* need this special case */
                params = as_malloc(sizeof(*params));
                nparam = 0;
            }
            params[nparam] = NULL;
            *params_array = params;
         ...
         * same name.
         */
        list_for_each(m, m->next) if (!mstrcmp(m->name, tline->text, m->casesense)) break;
    }
...
static void push_mmacro(MMacro *m)
{
    ...
    i = as_malloc(sizeof(MMacroInvocation));
    i->prev = m->prev;
    i->params = m->params;
    i->iline = m->iline;
    i->nparam = m->nparam;
...
static void verror(int severity, const char *fmt, va_list arg)
{
    ...
    if (mmac)
        as_error(severity, "(%s:%d) %s",
                 mmac->name, mmac->lineno - delta, buff);
    else
        as_error(severity, "%s", buff);
}
...
pp_reset(char *file, int apass, ListGen *listgen, StrList **deplist)
{
    ...
    {
        stdmacpos = as_stdmac_after_tasm;
    }
    ...
    while (1)
    {
        ...
        while (istk->expansion && istk->expansion->finishes)
        {
            ...
            else
            {
                ...
                /*
                 * TODO:  investigate the relationship at this point between
        ...
        { /* until we get a line we can use */
             ...
             */
            {
                ...
                if (i->conds)
                {
                    /* as_error can't be conditionally suppressed */
                    as_error(ERR_FATAL,
                ...
                if (i->next)
                {
                    ...
                    as_free(src_set_fname(as_strdup(i->fname)));
                }
                istk = i->next;
         ...
         * anything.
         */
        if (!defining && !(istk->conds && !emitting(istk->conds->state)) && !(istk->mstk && !istk->mstk->in_progress))
        {
...
void pp_include_path(char *path)
{
    ...
    i = as_malloc(sizeof(IncPath));
    i->path = path ? as_strdup(path) : NULL;
    i->next = NULL;
