/*
 * assemble.c   code generation for the assembler
 *
...
static uint32_t cpu; /* cpu level received from as.c */
                 ...
                 ListGen *listgen)
{
    ...
    if (instruction->opcode == I_INCBIN)
    {
        ...
        else
        {
            ...
            fclose(fp);
            return (int64_t)instruction->times * len;
        }
                  ...
                  insn *instruction, efunc error)
{
        ...
        instruction->opcode == I_DY)
    {
        ...
        return (int64_t)isize * instruction->times;
    }
    ...
    if (instruction->opcode == I_INCBIN)
    {
        ...
        else
        {
            ...
            val = (int64_t)instruction->times * len;
        }
        if (fp)
            fclose(fp);
        return val;
                        ...
                        insn *ins, const uint8_t *codes)
{
    ...
    while (*codes)
    {
        ...
        case 07:
            ...
            case4(014) : case4(020) : case4(024) : length++;
            break;

            case4(030) : length += 2;
            break;

            case4(034) : if (opx->type & (BITS16 | BITS32 | BITS64))
                             length += (opx->type & BITS16) ? 2 : 4;
            else length += (bits == 16) ? 2 : 4;
            break;

            case4(040) : length += 4;
            break;

            case4(044) : length += ins->addr_size >> 3;
            break;

            case4(050) : length++;
            break;

            case4(054) : length += 8; /* MOV reg64/imm */
            break;

            case4(060) : length += 2;
            break;

            case4(064) : if (opx->type & (BITS16 | BITS32 | BITS64))
                             length += (opx->type & BITS16) ? 2 : 4;
            else length += (bits == 16) ? 2 : 4;
            break;
            ...
            case4(0140) : length += is_sbyte16(opx) ? 1 : 2;
            break;
            ...
            case4(0150) : length += is_sbyte32(opx) ? 1 : 4;
            break;
        ...
        case 0174:
            ...
            case4(0250) : length += is_sbyte32(opx) ? 1 : 4;
            break;

            case4(0254) : length += 4;
            break;

            case4(0260) : ins->rex |= REX_V;
            ins->drexdst = regval(opx);
            ins->vex_cm = *codes++;
            ins->vex_wlp = *codes++;
            break;
        ...
        case 0373:
            ...
            case4(0100) : case4(0110) : case4(0120) : case4(0130) : case4(0200) : case4(0204) : case4(0210) : case4(0214) : case4(0220) : case4(0224) : case4(0230) : case4(0234) :
            {
                ea ea_data;
                int rfield;
                opflags_t rflags;
        ...
        default:
            errfunc(ERR_PANIC, "internal instruction table corrupt"
                               ": instruction code \\%o (0x%02X) given",
                    c, c);
            break;
        }
    }
                    ...
                    int64_t insn_end)
{
    ...
    while (*codes)
    {
        ...
        case 07:
            ...
            case4(020) : if (opx->offset < -256 || opx->offset > 255)
            {
                errfunc(ERR_WARNING | ERR_PASS2 | ERR_WARN_NOV,
            ...
            case4(024) : if (opx->offset < 0 || opx->offset > 255)
                             errfunc(ERR_WARNING | ERR_PASS2 | ERR_WARN_NOV,
            ...
            case4(030) : warn_overflow_opd(opx, 2);
            data = opx->offset;
            ...
            case4(034) : if (opx->type & (BITS16 | BITS32))
                             size = (opx->type & BITS16) ? 2 : 4;
            else size = (bits == 16) ? 2 : 4;
            ...
            case4(040) : warn_overflow_opd(opx, 4);
            data = opx->offset;
            ...
            case4(050) : if (opx->segment != segment)
            {
                data = opx->offset;
            ...
            case4(054) : data = (int64_t)opx->offset;
            out(offset, segment, &data, OUT_ADDRESS, 8,
                opx->segment, opx->wrt);
            offset += 8;
            break;

            case4(060) : if (opx->segment != segment)
            {
                data = opx->offset;
                out(offset, segment, &data,
            ...
            case4(070) : if (opx->segment != segment)
            {
                data = opx->offset;
                out(offset, segment, &data,
            ...
            case4(074) : if (opx->segment == NO_SEG)
        ...
        case 0172:
            c = *codes++;
            opx = &ins->oprs[c >> 3];
            bytes[0] = as_regvals[opx->basereg] << 4;
            opx = &ins->oprs[c & 7];
        ...
        case 0320:
        {
            ...
            if (pfx != P_O16 && pfx != P_none)
                as_error(ERR_WARNING, "Invalid operand size prefix");
...
static int op_rexflags(const operand *o, int mask)
{
    ...
    flags = as_reg_flags[o->basereg];
    val = as_regvals[o->basereg];
                                    ...
                                    int32_t segment, int64_t offset, int bits)
{
    ...
    for (temp = as_instructions[instruction->opcode];
...
{
    bool forw_ref = (input->opflags & OPFLAG_UNKNOWN) != 0;
    ...
    { /* it's a memory reference */
            ...
            (input->indexreg == -1 || input->scale == 0))
        {
            ...
            {
                as_error(ERR_WARNING | ERR_PASS1, "absolute address can not be RIP-relative");
                input->type &= ~IP_REL;
                input->type |= MEMORY;
            }
            ...
            {
                as_error(ERR_WARNING | ERR_PASS1, "displacement size ignored on absolute address");
            }
    ...
    for (j = 0; j < ins->operands; j++)
    {
        if (is_class(MEMORY, ins->oprs[j].type))
        {
            ...
            if (!is_register(ins->oprs[j].indexreg))
                ...
                i = as_reg_flags[ins->oprs[j].indexreg];

            /* Verify as Register */
            if (!is_register(ins->oprs[j].basereg))
                b = 0;
            else
                b = as_reg_flags[ins->oprs[j].basereg];
