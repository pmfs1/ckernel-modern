/*
 * outdbg.c	output routines for the assembler to produce
 *		a debugging trace
 */
...
{
    dbgsect = NULL;
    fprintf(ofile, "AS Output format debug dump\n");
}
                         ...
                         int is_global, char *special)
{
    fprintf(ofile, "deflabel %s := %08" PRIx32 ":%016" PRIx64 " %s (%d)%s%s\n",
            name, segment, offset,
            is_global == 2 ? "common" : is_global ? "global"
                                                  : "local",
