...
static int pe_section_class(Section *s)
{
    ...
    else
    {
        ...
        if (strncmp(name, ".stab", 5) == 0)
            return sec_stab; // .stab and .stabstr
    }
    return -1;
}
...
static int pe_write(struct pe_info *pe)
{
    ...
    if (pe->s1->noshare)
        pe_filehdr.Characteristics |= 0x4000;
...
int pe_load_def_file(CCState *s1, int fd)
{
    ...
    for (;;)
    {
        ...
        case 2:
            {
                size_t dllname_size = strlen(dllname) + 1;
                dllref = cc_malloc(sizeof(DLLReference) + dllname_size);
                strncpy(dllref->name, dllname, dllname_size);
                dllref->name[dllname_size - 1] = '\0';
                dllref->level = 0;
...
int pe_output_file(CCState *s1, const char *filename)
{
    ...
    // Generate relocation information by default for ckernel.
    ...
    if (!s1->nofll)
        pe_eliminate_unused_sections(&pe);
    ...
    if (s1->mapfile)
        pe_print_sections(s1, s1->mapfile);
