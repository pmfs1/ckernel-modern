...
int cc_get_symbol(CCState *s, unsigned long *pval, const char *name) {
    int sym_index;
    Elf32_Sym *sym;
...
void *cc_get_symbol_err(CCState *s, const char *name) {
    unsigned long val;
    if (cc_get_symbol(s, &val, name) < 0) error("%s not defined", name);
    return (void *) val;
}
    ...
    if (sym_bind != STB_LOCAL) {
        ...
        if (esym->st_shndx != SHN_UNDEF) {
            ...
            if (sh_num == SHN_UNDEF) {
                ...
                // Happens with 'cc ... -static cctest.c' on e.g. Ubuntu 6.01
                // No idea if this is the correct solution ...
                goto do_patch;
            } else if (s == cc_state->dynsymtab_section) {
...
void put_elf_reloc(Section *symtab, Section *s, unsigned long offset, int type, int symbol) {
    ...
    if (!sr) {
        ...
        sr = new_section(cc_state, buf, SHT_REL, symtab->sh_flags);
...
// the global and weak ones. Since CC cannot sort it while generating
// the code, we must do it after. All the relocation tables are also
// modified to take into account the symbol table sorting

void sort_syms(CCState *s1, Section *s) {
    int *old_to_new_syms;
    Elf32_Sym *new_syms;
    int nb_syms, i;
    Elf32_Sym *p, *q;
    ...
    nb_syms = s->data_offset / sizeof(Elf32_Sym);
    new_syms = cc_malloc(nb_syms * sizeof(Elf32_Sym));
    old_to_new_syms = cc_malloc(nb_syms * sizeof(int));
...
void relocate_syms(CCState *s1, int do_resolve) {
...
void relocate_section(CCState *s1, Section *s) {
    Section *sr;
    ...
    for (rel = qrel; rel < rel_end; rel++) {
        ...
        switch (type) {
            case R_386_32:
                if (s1->output_type == CC_OUTPUT_DLL) {
...
void relocate_rel(CCState *s1, Section *sr) {
    Section *s;
    Elf32_Rel *rel, *rel_end;
...
int prepare_dynamic_rel(CCState *s1, Section *sr) {
    Elf32_Rel *rel, *rel_end;
...
void put_got_offset(CCState *s1, int index, unsigned long val) {
    int n;
    unsigned long *tab;
    ...
    if (index >= s1->nb_got_offsets) {
        ...
        n = 1;
        while (index >= n) n *= 2;
        tab = cc_realloc(s1->got_offsets, n * sizeof(unsigned long));
...
void build_got(CCState *s1) {
    unsigned char *ptr;

    // If no got, then create it
    s1->got = new_section(s1, ".got", SHT_PROGBITS, SHF_ALLOC | SHF_WRITE);
    s1->got->sh_entsize = 4;
    add_elf_sym(symtab_section, 0, 4, ELF32_ST_INFO(STB_GLOBAL, STT_OBJECT), 0, s1->got->sh_num,
...
void put_got_entry(CCState *s1, int reloc_type, unsigned long size, int info, int sym_index) {
    int index;
    const char *name;
    Elf32_Sym *sym;
    ...
    if (s1->dynsym) {
        ...
        if (reloc_type == R_386_JMP_SLOT) {
            ...
            if (s1->output_type == CC_OUTPUT_EXE) offset = plt->data_offset - 16;
        }
...
void build_got_entries(CCState *s1) {
    Section *s, *symtab;
    Elf32_Rel *rel, *rel_end;
    Elf32_Sym *sym;
...
void add_init_array_defines(CCState *s1, const char *section_name) {
    Section *s;
    long end_offset;
    char sym_start[1024];
    char sym_end[1024];
...
// Add CC runtime libraries
void cc_add_runtime(CCState *s1) {
    char buf[1024];

    // Add libc
    if (!s1->nostdlib) {
        cc_add_library(s1, "c");
        snprintf(buf, sizeof(buf), "%s/%s", cc_lib_path, "libcc1.a");
        cc_add_file(s1, buf);
    }

    // Add crt end
    if (!s1->nostdlib) {
        cc_add_file(s1, CONFIG_CC_CRT_PREFIX "/crtn.o");
    }
}
...
// symbols))
void cc_add_linker_symbols(CCState *s1) {
    char buf[1024];
    int i;
    Section *s;
...
void cc_output_binary(CCState *s1, FILE *f, const int *section_order) {
    Section *s;
    int i, offset, size;
...
int cc_output_file(CCState *s1, const char *filename) {
    Elf32_Ehdr ehdr;
    FILE *f;
    int fd, mode, ret;
    int *section_order;
    ...
    if (file_type != CC_OUTPUT_OBJ) {
        ...
        if (!s1->static_link) {
            ...
            if (file_type == CC_OUTPUT_EXE) {
                for (sym = (Elf32_Sym *) symtab_section->data + 1; sym < sym_end; sym++) {
                    if (sym->st_shndx == SHN_UNDEF) {
                        ...
                        if (sym_index) {
                            ...
                            if (type == STT_FUNC) {
                                put_got_entry(s1, R_386_JMP_SLOT, esym->st_size, esym->st_info,
                ...
                s1->symtab_to_dynsym = cc_mallocz(sizeof(int) * nb_syms);
                for (sym = (Elf32_Sym *) symtab_section->data + 1; sym < sym_end; sym++) {
                    if (ELF32_ST_BIND(sym->st_info) != STB_LOCAL) {
                        name = symtab_section->link->data + sym->st_name;
                        index = put_elf_sym(s1->dynsym, sym->st_value, sym->st_size, sym->st_info, 0, sym->st_shndx,
                                            name);
    ...
    section_order = cc_malloc(sizeof(int) * shnum);
    section_order[0] = 0;
    sh_order_index = 1;
    ...
    for (i = 1; i < s1->nb_sections; i++) {
        ...
        if (file_type == CC_OUTPUT_DLL && s->sh_type == SHT_REL && !(s->sh_flags & SHF_ALLOC)) {
            prepare_dynamic_rel(s1, s);
        } else if (do_debug || file_type == CC_OUTPUT_OBJ || (s->sh_flags & SHF_ALLOC) ||
                   i == (s1->nb_sections - 1)) {
    ...
    phdr = cc_mallocz(phnum * sizeof(Elf32_Phdr));

    if (s1->output_format == CC_OUTPUT_FORMAT_ELF) {
    ...
    if (file_type != CC_OUTPUT_OBJ) {
        ...
        if (file_type == CC_OUTPUT_EXE) {
            ehdr.e_entry = (unsigned long) cc_get_symbol_err(s1, "_start");
        } else {
    ...
    if (s1->output_format == CC_OUTPUT_FORMAT_ELF) {
        ...
        cc_output_binary(s1, f, section_order);
    }
    fclose(f);

    ret = 0;
    cleanup:
    cc_free(s1->symtab_to_dynsym);
    cc_free(section_order);
    cc_free(phdr);
    cc_free(s1->got_offsets);
    return ret;
}
...
int cc_load_object_file(CCState *s1, int fd, unsigned long file_offset) {
    Elf32_Ehdr ehdr;
    Elf32_Shdr *shdr, *sh;
    ...
    sm_table = cc_mallocz(sizeof(SectionMergeInfo) * ehdr.e_shnum);
    ...
    // Resolve symbols
    old_to_new_syms = cc_mallocz(nb_syms * sizeof(int));
    sym = symtab + 1;
    for (i = 1; i < nb_syms; i++, sym++) {
        ...
        // Add symbol
        name = strtab + sym->st_name;
        sym_index = add_elf_sym(symtab_section, sym->st_value, sym->st_size, sym->st_info, sym->st_other, sym->st_shndx,
                                name);
...
int cc_load_alacarte(CCState *s1, int fd, int size) {
        ...
        for (p = ar_names, i = 0; i < nsyms; i++, p += strlen(p) + 1) {
            ...
            if (sym_index) {
                ...
                if (sym->st_shndx == SHN_UNDEF) {
                    ...
                    if (cc_load_object_file(s1, fd, off) < 0) {
                        fail:
                        ret = -1;
                        goto cleanup;
                    }
...
// Load a '.a' file
int cc_load_archive(CCState *s1, int fd) {
    ArchiveHeader hdr;
    char ar_size[11];
    char ar_name[17];
    char magic[8];
    ...
    for (;;) {
        ...
        if (!strcmp(ar_name, "/")) {
            // COFF symbol table
            if (s1->alacarte_link) return cc_load_alacarte(s1, fd, size);
            ...
            if (cc_load_object_file(s1, fd, file_offset) < 0) return -1;
        }
...
int cc_load_dll(CCState *s1, int fd, const char *filename, int level) {
    Elf32_Ehdr ehdr;
    ...
    for (i = 0, dt = dynamic; i < nb_dts; i++, dt++) {
        switch (dt->d_tag) {
            case DT_NEEDED:
                ...
                if (cc_add_dll(s1, name, AFF_REFERENCED_DLL) < 0) {
...
static int ld_next(CCState *s1, char *name, int name_size) {
    int c;
    char *q;
...
int ld_add_file_list(CCState *s1, int as_needed) {
    char filename[1024];
    int t, ret;
    ...
    for (;;) {
        ...
        if (!strcmp(filename, "AS_NEEDED")) {
            ...
            if (!as_needed) cc_add_file(s1, filename);
        }
        t = ld_next(s1, filename, sizeof(filename));
        if (t == ',') {
            t = ld_next(s1, filename, sizeof(filename));
...
int cc_load_ldscript(CCState *s1) {
    char cmd[64];
    char filename[1024];
    int t, ret;
