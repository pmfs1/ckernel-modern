        ...
        else
        {
            ...
            while (tok != '}')
            {
                ...
                while (1)
                {
                    ...
                    {
                        if (align > *cc_state->pack_stack_ptr)
                            align = *cc_state->pack_stack_ptr;
                    }
...
tok_next:
    ...
    case TOK_STR:
        ...
        t = VT_BYTE;
    str_init:
        if (cc_state->warn_write_strings)
            t |= VT_CONSTANT;
        type.t = t;
        mk_pointer(&type);
        type.t |= VT_ARRAY;
...
void gen_function(Sym *sym)
{
    ...
    if (do_debug)
        put_stabn(N_FUN, 0, 0, cur_text_section->data_offset - func_start);
...
CCState *cc_new(void)
{
    ...
    cc_define_symbol(s, "CKERNEL", NULL);
