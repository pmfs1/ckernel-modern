...
vfs_blkno_t set_inode_block(struct inode *inode, unsigned int iblock, vfs_blkno_t block)
{
    ...
    else
    {
        ...
        for (d = 1; d < inode->desc->depth; d++)
        {
            ...
            if (!buf)
                buf = get_buffer(inode->fs->cache, dirblock);
            goal = dirblock + 1;
        ...
        if (block == -1)
        {
            block = new_block(inode->fs,
                              offsets[d] == 0 ? dirblock + 1 : ((vfs_blkno_t *)buf->data)[offsets[d] - 1] + 1);
            inode->desc->blocks++;
            mark_inode_dirty(inode);
        }
...
struct inode *alloc_inode(struct inode *parent, int mode)
{
    ...
    block = inode->fs->groups[group].desc->inode_table_block +
            (ino % inode->fs->super->inodes_per_group) / inode->fs->inodes_per_block;
...
{
    if (inode->buf)
        release_buffer(inode->fs->cache, inode->buf);
    free(inode);
}
...
static void remove_blocks(struct filsys *fs, vfs_blkno_t *blocks, int count)
{
    ...
    {
        for (i = 0; i < count; i++)
            invalidate_buffer(fs->cache, blocks[i]);
...
int truncate_inode(struct inode *inode, unsigned int blocks)
{
    ...
    // Check arguments
    if (blocks > inode->desc->blocks)
        return -1;

    // Check for no-op case
    if (blocks == inode->desc->blocks)
        return 0;
    if (inode->desc->blocks == 0)
        return 0;
    ...
    while (blocksleft > 0)
    {
        ...
        for (d = 1; d <= inode->desc->depth; d++)
        {
            ...
            {
                if (buf[d])
                    release_buffer(inode->fs->cache, buf[d]);
