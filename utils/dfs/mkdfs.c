...
int dev_read(vfs_devno_t devno, void *buffer, size_t count, vfs_blkno_t blkno)
{
    ...
    if (bdrv_read(blkdev, blkno + part_offset, buffer, count / SECTORSIZE) < 0)
        panic("error reading from device");
    return count;
}
...
{
    struct blockdevice *blkdev = (struct blockdevice *)devno;
    if (bdrv_write(blkdev, blkno + part_offset, buffer, count / SECTORSIZE) < 0)
...
void read_boot_sector(char *name)
{
    ...
    if (fd == -1)
        panic("unable to read boot sector");

    bytes = read(fd, bootsect, SECTORSIZE);
    if (bytes != SECTORSIZE)
        panic("bad bootsector");
...
void read_mbr(struct blockdevice *bs)
{
    ...
    if (bdrv_read(bs, 0, (unsigned char *)&mbr, 1) < 0)
        panic("error reading mbr from device");

    if (mbr.signature != MBR_SIGNATURE)
        panic("invalid master boot record");
...
void install_loader()
{
    ...
    if (fd == -1)
        panic("unable to read os loader");
    size = filesize(fd);
    image = malloc(size);
    count = read(fd, image, size);
    close(fd);
    if (count != size)
        panic("error reading os loader");
    ...
    if (blocks > fs->super->reserved_blocks)
        panic("not enough space for os loader");

    for (i = 0; i < blocks; i++)
    {
        dev_write(fs->devno, image + i * fs->blocksize, fs->blocksize,
                  (fs->super->first_reserved_block + i) * (fs->blocksize / SECTORSIZE));
    }
...
void transfer_file(char *dstfn, char *srcfn)
{
    ...
    while (ext > dstfn && *ext != '.' && *ext != PS1 && *ext != PS2)
        ext--;
    if (strcmp(ext, ".dll") == 0 ||
        strcmp(ext, ".exe") == 0 ||
        strcmp(ext, ".sys") == 0 ||
...
int main(int argc, char **argv)
{
    ...
    // Open device file
    if (bdrv_open(&blkdev, devname, NULL) < 0)
    ...
    if (doformat)
    {
        ...
        if (vfs_format((vfs_devno_t)&blkdev, "dfs", options) < 0)
            panic("error formatting device");
    }

    // Initialize the file system
    printf("Mounting device\n");
    if (vfs_mount("dfs", "/", (vfs_devno_t)&blkdev, NULL) < 0)
        panic("error mounting device");
