...
static int vmdk_open(struct blockdevice *bs, const char *filename)
{
    ...
    if (seek(fd, s->l1_table_offset) == -1)
        goto fail;
    ...
    if (s->l1_backup_table_offset)
    {
        ...
        if (seek(fd, s->l1_backup_table_offset) == -1)
            goto fail;
...
static uint64_t get_cluster_offset(struct blockdevice *bs, uint64_t offset, int allocate)
{
    ...
    pos = l2_offset * 512;
    seek(s->fd, pos);
    if (read(s->fd, l2_table, s->l2_size * sizeof(uint32_t)) != s->l2_size * sizeof(uint32_t))
        return 0;
