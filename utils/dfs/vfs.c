...
int vfs_mount(char *type, char *path, vfs_devno_t devno, char *opts)
{
    ...
    while (fs)
    {
        if (fnmatch(path, strlen(path), fs->path, strlen(fs->path)))
            return -1;
        fs = fs->next;
    }
...
int vfs_write(struct file *filp, void *data, size_t size)
{
    ...
    if (!data && size > 0)
        return -1;
    if (filp->flags & VFS_O_RDONLY)
        return -1;

    if (!filp->fs->ops->write)
        return -1;
