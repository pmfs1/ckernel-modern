...
static void insert_into_hashtable(struct bufpool *pool, struct buf *buf)
{
    ...
    if (pool->hashtable[slot])
        pool->hashtable[slot]->bucket.prev = buf;
...
static void remove_from_hashtable(struct bufpool *pool, struct buf *buf)
{
    ...
    slot = bufhash(buf->blkno) % BUFPOOL_HASHSIZE;
    if (buf->bucket.next)
        buf->bucket.next->bucket.prev = buf->bucket.prev;
    if (buf->bucket.prev)
        buf->bucket.prev->bucket.next = buf->bucket.next;
    if (pool->hashtable[slot] == buf)
        pool->hashtable[slot] = buf->bucket.next;
...
static struct buf *lookup_buffer(struct bufpool *pool, vfs_blkno_t blkno)
{
    ...
    case BUF_STATE_CLEAN:
        ...
        if (buf->chain.next)
            buf->chain.next->chain.prev = buf->chain.prev;
        if (buf->chain.prev)
            buf->chain.prev->chain.next = buf->chain.next;
        if (pool->clean.head == buf)
            pool->clean.head = buf->chain.next;
        if (pool->clean.tail == buf)
            pool->clean.tail = buf->chain.prev;
    ...
    case BUF_STATE_DIRTY:
        ...
        if (buf->chain.prev)
            buf->chain.prev->chain.next = buf->chain.next;
        if (pool->dirty.head == buf)
            pool->dirty.head = buf->chain.next;
        if (pool->dirty.tail == buf)
            pool->dirty.tail = buf->chain.prev;
...
static struct buf *get_new_buffer(struct bufpool *pool)
{
    ...
    while (1)
    {
        ...
        if (pool->clean.head)
        {
            ...
            buf = pool->clean.head;
            if (buf->chain.next)
                buf->chain.next->chain.prev = NULL;
            pool->clean.head = buf->chain.next;
            if (pool->clean.tail == buf)
                pool->clean.tail = buf->chain.next;
...
void release_buffer(struct bufpool *pool, struct buf *buf)
{
    ...
    case BUF_STATE_LOCKED:
        ...
        if (pool->clean.tail)
            pool->clean.tail->chain.next = buf;
        pool->clean.tail = buf;
        if (!pool->clean.head)
            pool->clean.head = buf;
        break;
    ...
    case BUF_STATE_UPDATED:
        ...
        if (pool->dirty.tail)
            pool->dirty.tail->chain.next = buf;
        pool->dirty.tail = buf;
        if (!pool->dirty.head)
            pool->dirty.head = buf;
        break;
