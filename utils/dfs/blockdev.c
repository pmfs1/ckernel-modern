...
#ifdef WIN32
    if (_lseeki64(s->fd, pos, SEEK_SET) == -1)
        return -1;
#else
    if (lseek(s->fd, pos, SEEK_SET) == -1)
        return -1;
#endif
    if (read(s->fd, buf, nb_sectors * 512) != nb_sectors * 512)
        return -1;
    return 0;
}
...
static struct blockdriver *get_driver(char *type)
{
    ...
    for (i = 0; drivers[i]; i++)
    {
        if (strcmp(drivers[i]->format_name, type) == 0)
            return drivers[i];
    }
    return NULL;
}
