...
int read_boot_sector(HANDLE hdev)
{
    ...
    if (SetFilePointer(hdev, 0, NULL, FILE_BEGIN) == -1)
        return -1;
    if (!ReadFile(hdev, bootsect, 512, &bytes, NULL))
        return -1;
...
int read_mbr_file(char *filename)
{
    ...
    if (!ReadFile(himg, bootsect, 512, &bytes, NULL))
        return -1;
    CloseHandle(himg);
    return 0;
}
...
int write_boot_sector(HANDLE hdev)
{
    ...
    if (SetFilePointer(hdev, 0, NULL, FILE_BEGIN) == -1)
        return -1;
    if (!WriteFile(hdev, bootsect, 512, &bytes, NULL))
        return -1;
...
int main(int argc, char **argv)
{
    ...
    // Open device file
    hdev = CreateFile(devname, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
