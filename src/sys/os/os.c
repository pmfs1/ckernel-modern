...
static void *load_image(char *filename) {
    ...
    imgbase = (char *) vmalloc(NULL, imghdr->optional.size_of_image, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE,
                               'UMOD');
    if (imgbase == NULL) {
        close(f);
        free(buffer);
    ...
    for (i = 0; i < imghdr->header.number_of_sections; i++) {
        if (imghdr->sections[i].pointer_to_raw_data != 0) {
            if (lseek(f, imghdr->sections[i].pointer_to_raw_data, SEEK_SET) !=
                imghdr->sections[i].pointer_to_raw_data) {
            ...
            if (read(f, RVA(imgbase, imghdr->sections[i].virtual_address), imghdr->sections[i].size_of_raw_data) !=
                (int) imghdr->sections[i].size_of_raw_data) {
                vmfree(imgbase, imghdr->optional.size_of_image, MEM_RELEASE);
                close(f);
                free(buffer);
                return NULL;
...
int uname(struct utsname *buf) {
    ...
    sprintf(buf->release, "%d.%d.%d.%d", ver->file_major_version, ver->file_minor_version, ver->file_release_number,
            ver->file_build_number);
    sprintf(buf->version, "%s%04d-%02d-%02d %02d:%02d:%02d", build, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
...
int __stdcall start(hmodule_t hmod, void *reserved, void *reserved2) {
    ...
    usermods.log = logldr;
    init_module_database(&usermods, "os.dll", hmod, get_property(config, "os", "libpath", "/bin"),
                         find_section(config, "modaliases"), 0);
