...
static int pnp_bios_dev_node_info(struct pnp_dev_node_info *data) {
    ...
    set_gdt_entry(GDT_AUX1, (unsigned long) data, sizeof(struct pnp_dev_node_info),
                  D_DATA | D_DPL0 | D_WRITE | D_PRESENT, 0);
...
static int pnp_bios_isapnp_config(struct pnp_isa_config_struc *data) {
    ...
    set_gdt_entry(GDT_AUX1, (unsigned long) data, sizeof(struct pnp_isa_config_struc),
                  D_DATA | D_DPL0 | D_WRITE | D_PRESENT, 0);
...
static int pnp_bios_escd_info(struct escd_info_struc *data) {
    ...
    set_gdt_entry(GDT_AUX1, (unsigned long) data, sizeof(struct escd_info_struc), D_DATA | D_DPL0 | D_WRITE | D_PRESENT,
                  0);
...
static void build_sys_devlist(struct bus *bus) {
    ...
    for (i = 0; i < 0xFF && nodenum != 0xFF; i++) {
        ...
        if (pnp_bios_get_dev_node((unsigned char *) &nodenum, (char) 0 /*1*/, node)) {
            kprintf(KERN_ERR
            "pnpbios: PnP BIOS reported error on attempt to get dev node.\n");
            break;
        }
...
static void parse_escd(struct bus *bus, unsigned char *b, int maxlen) {
    ...
    if (memcmp("ACFG", &b[2], 4) != 0) {
        ...
        "escd: signature does not match ACFG, is: %4s\n", &b[2]);
        return;
    }
...
int enum_isapnp(struct bus *bus) {
    ...
    for (ofs = 0xF0000; ofs < 0xFFFF0; ofs += 16) {
        ...
        set_gdt_entry(GDT_PNPTHUNK, (unsigned long) pnp_bios_thunk, 1, D_CODE | D_DPL0 | D_READ | D_PRESENT,
                      D_BIG | D_BIG_LIM);
