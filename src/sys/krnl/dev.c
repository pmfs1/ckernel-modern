...
struct resource *
add_resource(struct unit *unit, unsigned short type, unsigned short flags, unsigned long start, unsigned long len) {
    struct resource *res;
    struct resource *r;
...
static int initialize_driver(struct unit *unit, char *driverstr) {
    ...
    if (!entry) {
        kprintf(KERN_ERR
        "dev: unable to find entry %s in module %s\n", entryname, modname);
        unload(hmod);
        kfree(buf);
        return -ENOEXEC;
    }
    ...
    if (rc < 0) {
        kprintf(KERN_ERR
        "dev: initialization of %s!%s failed with error %d\n", modname, entryname, rc);
...
static void install_driver(struct unit *unit, struct binding *bind) {
    ...
    if (rc < 0) {
        kprintf(KERN_ERR
        "dev: driver '%s' failed with error %d for unit %08X '%s'\n", bind->module, rc, unit->unitcode, get_unit_name(
                unit));
    }
}
...
static void install_legacy_drivers() {
    ...
    while (prop) {
        ...
        if (buf) {
            ...
            if (rc < 0) {
                kprintf(KERN_ERR
                "dev: error %d initializing driver %s\n", rc, prop->name);
            }
...
static int units_proc(struct proc_file *pf, void *arg) {
    ...
    while (unit) {
        ...
        pprintf(pf, "%s unit %d.%d class %08X code %08X %s:\n", busnames[bustype], busno, unit->unitno, unit->classcode,
                unit->unitcode, get_unit_name(unit));
