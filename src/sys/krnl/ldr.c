...
void *load_image_file(char *filename, int userspace) {
    ...
    if (userspace) {
        // User module
        imgbase = (char *) vmalloc((void *) (imghdr->optional.image_base), imghdr->optional.size_of_image,
                                   MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE, 'UMOD', NULL);
        if (imgbase == NULL) {
            // Try to load image at any available address
            imgbase = (char *) vmalloc(NULL, imghdr->optional.size_of_image, MEM_RESERVE | MEM_COMMIT,
                                       PAGE_EXECUTE_READWRITE, 'UMOD', NULL);
        }
    } else {
        // Kernel module
...
void init_kernel_modules() {
    ...
    init_module_database(&kmods, "krnl.dll", (hmodule_t) OSBASE, get_property(krnlcfg, "kernel", "libpath", "/boot"),
                         find_section(krnlcfg, "modaliases"), 0);
