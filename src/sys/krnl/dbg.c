...
static void dbg_get_modules(struct dbg_hdr *hdr, union dbg_body *body) {
    ...
    body->modl.count = n;
    dbg_send_packet(hdr->cmd | DBGCMD_REPLY, hdr->id, body,
                    sizeof(struct dbg_modulelist) + n * sizeof(struct dbg_moduleinfo));
}
...
static void dbg_get_threads(struct dbg_hdr *hdr, union dbg_body *body) {
    ...
    body->thl.count = n;
    dbg_send_packet(hdr->cmd | DBGCMD_REPLY, hdr->id, body,
                    sizeof(struct dbg_threadlist) + n * sizeof(struct dbg_threadinfo));
}
