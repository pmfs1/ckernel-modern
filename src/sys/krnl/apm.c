...
int apm_enable_power_management(unsigned long enable) {
    ...
    rc = apm_bios_call_simple(APM_FUNC_ENABLE_PM, apm_conn_ver > 0x0100 ? APM_DEVICE_ALL : APM_DEVICE_OLD_ALL, enable,
                              &eax);
    if (rc != 0) return (eax >> 8) & 0xFF;
    return 0;
}
...
void apm_power_off() {
    ...
    if (apm_enabled) {
        ...
        if (rc != 0) {
            kprintf(KERN_ERR
            "apm: error %d in apm_set_system_power_state()\n", rc);
        }
    }
}
...
int __declspec(dllexport) apm(struct unit *unit, char *opts) {
    ...
    set_gdt_entry(GDT_APM40, (unsigned long) iomap(0x400, 4096), 4096 - 0x40 * 16,
                  D_DATA | D_DPL0 | D_WRITE | D_PRESENT, D_BIG);
    ...
    if (enable == 1 || enable == 2 && (apm->flags & APM_BIOS_DISABLED) != 0) {
        ...
        if (rc != 0) {
            kprintf(KERN_ERR
            "apm: error %d in apm_enable_power_management()\n", rc);
            return -EIO;
        }
    }
    ...
    if (engage == 1 || engage == 2 && (apm->flags & APM_BIOS_DISENGAGED) != 0) {
        ...
        if (rc != 0) {
            kprintf(KERN_ERR
            "apm: error %d in apm_engage_power_management()\n", rc);
            return -EIO;
        }
    }
