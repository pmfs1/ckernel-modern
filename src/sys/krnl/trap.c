...
static int div_handler(struct context *ctxt, void *arg) {
    if (usermode(ctxt)) {
        ...
        kprintf(KERN_CRIT
        "trap: division by zero in kernel mode\n");
        dbg_enter(ctxt, 0);
    }
...
static int breakpoint_handler(struct context *ctxt, void *arg) {
    if (usermode(ctxt)) {
        ...
        kprintf(KERN_CRIT
        "trap: breakpoint in kernel mode\n");
        dbg_enter(ctxt, 0);
    }
...
static int overflow_handler(struct context *ctxt, void *arg) {
    if (usermode(ctxt)) {
        ...
        kprintf(KERN_CRIT
        "trap: overflow exception in kernel mode\n");
        dbg_enter(ctxt, 0);
    }
...
static int bounds_handler(struct context *ctxt, void *arg) {
    if (usermode(ctxt)) {
        ...
        kprintf(KERN_CRIT
        "trap: bounds exception in kernel mode\n");
        dbg_enter(ctxt, 0);
    }
...
static int illop_handler(struct context *ctxt, void *arg) {
    if (usermode(ctxt)) {
        ...
        kprintf(KERN_CRIT
        "trap: illegal instruction exception in kernel mode\n");
        dbg_enter(ctxt, 0);
    }
...
static int seg_handler(struct context *ctxt, void *arg) {
    if (usermode(ctxt)) {
        ...
        kprintf(KERN_CRIT
        "trap: sement not present exception in kernel mode\n");
        dbg_enter(ctxt, 0);
    }
...
static int stack_handler(struct context *ctxt, void *arg) {
    if (usermode(ctxt)) {
        ...
        kprintf(KERN_CRIT
        "trap: stack segment exception in kernel mode\n");
        dbg_enter(ctxt, 0);
    }
...
static int genpro_handler(struct context *ctxt, void *arg) {
    if (usermode(ctxt)) {
        ...
        kprintf(KERN_CRIT
        "trap: general protection exception in kernel mode\n");
        dbg_enter(ctxt, 0);
    }
...
static int pagefault_handler(struct context *ctxt, void *arg) {
    ...
    if (usermode(ctxt)) {
        ...
        kprintf(KERN_CRIT
        "trap: page fault in kernel mode\n");
        dbg_enter(ctxt, addr);
    }
...
static int alignment_handler(struct context *ctxt, void *arg) {
    if (usermode(ctxt)) {
        ...
        kprintf(KERN_CRIT
        "trap: alignment exception in kernel mode\n");
        dbg_enter(ctxt, 0);
    }
