            ...
            if (classcode == PCI_BRIDGE) {
                ...
                for (bar = 0; bar < 6; bar++) {
                    ...
                    if (len != 0 && len != 0xFFFFFFFF) {
                        ...
                        if (value & 1) {
                            add_resource(unit, RESOURCE_IO, 0, value & PCI_BASE_ADDRESS_IO_MASK,
                                         pci_size(len, PCI_BASE_ADDRESS_IO_MASK & 0xFFFF));
                        } else {
                            add_resource(unit, RESOURCE_MEM, 0, value & PCI_BASE_ADDRESS_MEM_MASK,
                                         pci_size(len, PCI_BASE_ADDRESS_MEM_MASK));
                        }
                    }
                }
