...
int smb_mount(struct fs *fs, char *opts)
{
    ...
    if (!fs->mntfrom)
        return -EINVAL;
    if (strlen(fs->mntfrom) + 1 > SMB_NAMELEN)
        return -EINVAL;
...
int smb_open(struct file *filp, char *name)
{
    ...
    if (filp->flags & O_SHORT_LIVED)
        attrs |= SMB_FILE_ATTR_TEMPORARY;
    ...
    if (filp->flags & O_DIRECT)
        attrs |= SMB_FILE_FLAG_NO_BUFFERING | SMB_FILE_FLAG_WRITE_THROUGH;
...
int smb_read(struct file *filp, void *data, size_t size, off64_t pos)
{
    ...
    if (filp->pos < file->statbuf.st_size && pos < 0x80000000)
    {
        ...
        {
            count = left;
            if (count > SMB_RAW_CHUNKSIZE)
                count = SMB_RAW_CHUNKSIZE;
    ...
    {
        count = left;
        if (count > SMB_NORMAL_CHUNKSIZE)
            count = SMB_NORMAL_CHUNKSIZE;
...
int smb_write(struct file *filp, void *data, size_t size, off64_t pos)
{
    ...
    while (left > 0)
    {
        ...
        if (pos > file->statbuf.st_size)
            file->statbuf.st_size = pos;
    }
...
int smb_futime(struct file *filp, struct utimbuf *times)
{
    ...
    if (times->ctime != -1)
        file->statbuf.st_ctime = times->ctime;
    if (times->modtime != -1)
        file->statbuf.st_mtime = times->modtime;
    if (times->actime != -1)
        file->statbuf.st_atime = times->actime;
...
int smb_fstat(struct file *filp, struct stat64 *buffer)
{
    ...
    if (buffer)
        memcpy(buffer, &file->statbuf, sizeof(struct stat64));
...
int smb_stat(struct fs *fs, char *name, struct stat64 *buffer)
{
    ...
    {
        if (buffer)
            memcpy(buffer, &dentry->statbuf, sizeof(struct stat64));
