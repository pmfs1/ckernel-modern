...
static int cdfs_read_path_table(struct cdfs *cdfs, struct iso_volume_descriptor *vd) {
    ...
    // Allocate path table
    cdfs->path_table = (struct iso_pathtable_record **) kmalloc(
            cdfs->path_table_records * sizeof(struct iso_pathtable_record **));
...
static int cdfs_find_in_dir(struct cdfs *cdfs, int dir, char *name, int len, struct buf **dirbuf,
