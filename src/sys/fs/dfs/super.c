...
static struct filsys *create_filesystem(char *devname, struct fsoptions *fsopts) {
    ...
    fs->super->inodes_per_group =
            (fs->super->inodes_per_group + fs->inodes_per_block - 1) / fs->inodes_per_block * fs->inodes_per_block;
    ...
    if ((fsopts->flags & FSOPT_QUICK) == 0) {
        ...
        prev_percent = -1;
        for (i = fs->super->groupdesc_table_block + fs->groupdesc_blocks;
             i < fs->super->block_count; i += blocks_per_io) {
            int rc;
    ...
    for (i = 0; i < fs->super->reserved_inodes; i++) {
        ...
        if (ino != i) {
            kprintf(KERN_ERR
            "dfs: format expected inode %d, got %d\n", i, ino);
            return NULL;
        }
    }
...
static struct filsys *open_filesystem(char *devname, struct fsoptions *fsopts) {
    ...
    if (dev_read(devno, fs->super, SECTORSIZE, 1, 0) != SECTORSIZE) {
        kprintf(KERN_ERR
        "dfs: unable to read superblock on device %s\n", device(devno)->name);
        free(fs->super);
        free(fs);
        return NULL;
    }
    ...
    if (fs->super->signature != DFS_SIGNATURE) {
        kprintf(KERN_ERR
        "dfs: invalid DFS signature on device %s\n", device(devno)->name);
        free(fs->super);
        free(fs);
        return NULL;
    }
    ...
    if (fs->super->version != DFS_VERSION) {
        kprintf(KERN_ERR
        "dfs: invalid DFS version on device %s\n", device(devno)->name);
        free(fs->super);
        free(fs);
        return NULL;
    }
