        ...
        while (queue == NULL || left > 0) {
            ...
            if (optdata != NULL) {
                ...
                if ((seg->p = pbuf_alloc(PBUF_TRANSPORT, size, PBUF_RW)) == NULL) {
                    kprintf(KERN_ERR
                    "tcp_enqueue: could not allocate memory for pbuf copy\n");
                    goto memerr;
                }
...
static void tcp_output_segment(struct tcp_seg *seg, struct tcp_pcb *pcb) {
    ...
    if (seg->p->ref > 1) {
        kprintf(KERN_ERR
        "tcp_output_segment: packet not transmitted, already in tx queue\n");
        return;
    }
    ...
    if (seg->p->ref > 1) {
        kprintf(KERN_ERR
        "tcp_output_segment: packet not retransmitted, still in tx queue\n");
        return;
    }
    ...
    if (netif == NULL) {
        kprintf(KERN_ERR
        "tcp_output_segment: No route to %a\n", &pcb->remote_ip);
        stats.ip.rterr++;
        return;
    }
    ...
    seg->tcphdr->chksum = 0;
    if ((netif->flags & NETIF_TCP_TX_CHECKSUM_OFFLOAD) == 0) {
        seg->tcphdr->chksum = inet_chksum_pseudo(seg->p, &pcb->local_ip, &pcb->remote_ip, IP_PROTO_TCP,
                                                 seg->p->tot_len);
    }
    stats.tcp.xmit++;
...
void tcp_rst(unsigned long seqno, unsigned long ackno, struct ip_addr *local_ip, struct ip_addr *remote_ip,
