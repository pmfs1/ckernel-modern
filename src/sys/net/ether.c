...
struct netif *
ether_netif_add(char *name, char *devname, struct ip_addr *ipaddr, struct ip_addr *netmask, struct ip_addr *gw) {
    struct netif *netif;
    ...
    if (ip_addr_isany(ipaddr)) {
        ...
        if (state) {
            if (wait_for_object(&state->binding_complete, 30000) < 0) {
                ...
                "ether: timeout waiting for dhcp to complete on %s\n", name);
            }
        }
    }

    kprintf(KERN_INFO
    "%s: device %s addr %a mask %a gw %a\n", name, devname, &netif->ipaddr, &netif->netmask, &netif->gw);
...
int register_ether_netifs() {
    ...
    for (devno = 0; devno < num_devs; devno++) {
        ...
        if (rc < 0) kprintf(KERN_ERR
        "ether: unable to attach to device %s (error %d)\n", dev->name, rc);
    }
...
err_t ether_output(struct netif *netif, struct pbuf *p, struct ip_addr *ipaddr) {
    ...
    if (pbuf_header(p, ETHER_HLEN)) {
        kprintf(KERN_ERR
        "ether_output: not enough room for Ethernet header in pbuf\n");
        stats.link.err++;
        return -EBUF;
    }
    ...
    if (dest == NULL) {
        ...
        if (q != NULL) {
            ...
            if (err < 0) {
                kprintf(KERN_ERR
                "ether: error %d sending arp packet\n", err);
                pbuf_free(q);
                stats.link.drop++;
                return err;
            }
