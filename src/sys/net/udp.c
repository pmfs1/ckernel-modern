...
err_t udp_input(struct pbuf *p, struct netif *inp) {
    ...
    if (pbuf_header(p, -(IPH_HL(iphdr) * 4)) < 0 || p->tot_len < sizeof(struct udp_hdr)) {
        ...
        "udp_input: short packet (%u bytes) discarded\n", p->tot_len);
        stats.udp.lenerr++;
        stats.udp.drop++;
        return -EPROTO;
    }
...
err_t
udp_send(struct udp_pcb *pcb, struct pbuf *p, struct ip_addr *dst_ip, unsigned short dst_port, struct netif *netif) {
    struct udp_hdr *udphdr;
    ...
    if (pbuf_header(p, UDP_HLEN) < 0) {
        kprintf(KERN_ERR
        "udp_send: not enough room for UDP header in pbuf\n");
        stats.udp.err++;
        return -EBUF;
    }
