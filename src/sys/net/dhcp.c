...
static void dhcp_handle_nak(struct dhcp_state *state) {
    ...
    "dhcp_handle_nak: request timeout %u msecs\n", msecs);
...
static void dhcp_timeout_handler(struct dhcp_state *state) {
    if (state->state == DHCP_BACKING_OFF || state->state == DHCP_SELECTING) {
        ...
        if (state->tries <= 8) {
            ...
            struct netif *netif = state->netif;
            kprintf(KERN_WARNING
            "dhcp_timeout: REBINDING, release, restart\n");
            dhcp_release(state);
            dhcp_discover(state);
        }
    }
...
static void dhcp_bind(struct dhcp_state *state) {
    ...
    state->netif->broadcast.addr =
            (state->netif->ipaddr.addr & state->netif->netmask.addr) | ~(state->netif->netmask.addr);
                ...
                if (dhcp_unfold_reply(state) == 0) {
                    ...
                    if (options_ptr != NULL) {
                        ...
                        if (msg_type == DHCP_ACK) {
                            ...
                            } else if (state->state == DHCP_REBOOTING || state->state == DHCP_REBINDING ||
                                       state->state == DHCP_RENEWING) {
                                dhcp_bind(state);
                            }
