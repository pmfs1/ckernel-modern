...
int nic_transmit(struct dev *dev, struct pbuf *p) {
    ...
    if (wait_for_object(&nic->tx_sem, TX_TIMEOUT) < 0) {
        kprintf(KERN_ERR
        "nic: transmit timeout, drop packet\n");
        netstats->link.drop++;
        return -ETIMEOUT;
    }
    ...
    entry->header =
            FSH_ROUND_UP_DEFEAT | FSH_DOWN_INDICATE | FSH_TX_INDICATE | FSH_ADD_IP_CHECKSUM | FSH_ADD_TCP_CHECKSUM |
            FSH_ADD_UDP_CHECKSUM;
    entry->phys_next = 0;
    entry->pkt = p;
...
void nic_tx_complete(struct nic *nic) {
    ...
    if (txstatus & TX_STATUS_HWERROR) {
        ...
        kprintf(KERN_ERR
        "nic: maximum collisions\n");
        nic->stat.tx_maximum_collisions++;
        execute_command(nic, CMD_TX_ENABLE, 0);
    } else if (txstatus & 0x3F) {
        kprintf(KERN_ERR
        "nic: unknown errror in 0x%02X tx complete\n", txstatus);
        nic->stat.tx_unknown_error++;
        execute_command(nic, CMD_TX_ENABLE, 0);
    }
}
...
int nic_restart_transmitter(struct nic *nic) {
    ...
    if (media_status & MEDIA_STATUS_TX_IN_PROGRESS) {
        ...
        while (1) {
            ...
            if (time_after(get_ticks(), timeout)) {
                kprintf(KERN_WARNING
                "nic: timeout waiting for transmitter to go quiet\n");
                return -ETIMEOUT;
            }
            msleep(10);
        }
    ...
    if (dma_control & DMA_CONTROL_DOWN_IN_PROGRESS) {
        ...
        while (1) {
            ...
            if (time_after(get_ticks(), timeout)) {
                kprintf(KERN_WARNING
                "nic: timeout waiting for download engine to stop\n");
                return -ETIMEOUT;
            }
            msleep(10);
        }
...
int nic_configure_mii(struct nic *nic, unsigned short media_options) {
    ...
    if (!(phy_status & MII_STATUS_AUTO_DONE)) {
        ...
        while (1) {
            ...
            if (time_after(get_ticks(), timeout)) {
                kprintf(KERN_WARNING
                "nic: timeout waiting for auto-negotiation to finish\n");
                return -ETIMEOUT;
            }
            msleep(10);
        }
...
int nic_check_mii_auto_negotiation_status(struct nic *nic) {
    ...
    if ((phy_anar & MII_ANAR_MEDIA_MASK) != (phy_anlpar & MII_ANAR_MEDIA_MASK)) {
        kprintf(KERN_ERR
        "nic: incompatible configuration\n");
        return -EINVAL;
    }
...
int nic_check_dc_converter(struct nic *nic, int enabled) {
    ...
    if (enabled && !(media_status & MEDIA_STATUS_DC_CONVERTER_ENABLED) ||
        ...
        while (1) {
            ...
            if (time_after(get_ticks(), timeout)) {
                kprintf(KERN_ERR
                "nic: timeout waiting for dc converter to go %s\n", enabled ? "on" : "off");
                return -ETIMEOUT;
            }
            msleep(10);
        }
...
int nic_setup_connector(struct nic *nic, int connector) {
    ...
    if (connector == CONNECTOR_10BASET || connector == CONNECTOR_100BASETX) {
        ...
        if (nic_check_mii_configuration(nic, (unsigned short) (connector == CONNECTOR_100BASETX
                                                               ? MEDIA_OPTIONS_100BASETX_AVAILABLE
                                                               : MEDIA_OPTIONS_10BASET_AVAILABLE)) < 0) {
    ...
    media_status = inpw(nic->iobase + MEDIA_STATUS);
    media_status &= ~(MEDIA_STATUS_SQE_STATISTICS_ENABLE | MEDIA_STATUS_LINK_BEAT_ENABLE |
...
int nic_setup_media(struct nic *nic) {
    ...
    //
    if (nic->autoselect && nic->flowcontrol && nic->deviceid != 0x9200 && nic->deviceid != 0x9201 &&
        nic->deviceid != 0x9805) {
...
int __declspec(dllexport) install(struct unit *unit) {
    ...
    "%s: %s, iobase 0x%x irq %d mac %la\n", device(
            nic->devno)->name, unit->productname, nic->iobase, nic->irq, &nic->hwaddr);
    kprintf(KERN_INFO
    "%s: %d MBits/s, %s-duplex, %s\n", device(nic->devno)->name, nic->linkspeed, nic->fullduplex ? "full"
                                                                                                 : "half", connectorname[nic->connector]);
