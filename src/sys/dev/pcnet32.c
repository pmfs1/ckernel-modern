    ...
    while (!(pcnet32->rx_ring[entry].status & RMD_OWN)) {
        ...
        if (status != 0x03) {
            ...
            if (pcnet32->rx_buffer[entry]) {
                ...
                "pcnet32: unable to allocate packet for receive ring\n");
...
void pcnet32_dpc(void *arg) {
    ...
    while ((csr = pcnet32->read_csr(iobase, CSR)) & (CSR_ERR | CSR_RINT | CSR_TINT)) {
        ...
        pcnet32->write_csr(iobase, CSR,
                           (unsigned short) (csr & ~(CSR_IENA | CSR_TDMD | CSR_STOP | CSR_STRT | CSR_INIT)));
        //dump_csr(csr);
...
int __declspec(dllexport) install(struct unit *unit) {
    ...
    version = pcnet32->read_csr(pcnet32->iobase, CHIP_ID_LOWER) |
              (pcnet32_wio_read_csr(pcnet32->iobase, CHIP_ID_UPPER) << 16);
    ...
    for (i = 0; i < ETHER_ADDR_LEN; i++)
        pcnet32->hwaddr.addr[i] = (unsigned char) inp((unsigned short) (pcnet32->iobase + i));
    ...
    pcnet32->init_block.rx_ring = pcnet32->phys_addr + offsetof(
    struct pcnet32, rx_ring);
    pcnet32->init_block.tx_ring = pcnet32->phys_addr + offsetof(
    struct pcnet32, tx_ring);
    ...
    init_block = pcnet32->phys_addr + offsetof(
    struct pcnet32, init_block);
    ...
    kprintf(KERN_INFO
    "%s: %s iobase 0x%x irq %d mac %s\n", device(
            pcnet32->devno)->name, unit->productname, pcnet32->iobase, pcnet32->irq, ether2str(&pcnet32->hwaddr, str));
