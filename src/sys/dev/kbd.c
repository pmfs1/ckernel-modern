...
int reset_keyboard() {
    ...
    if (kbd_wait_for_data() != 0x00) {
        kprintf(KERN_WARNING
        "kbd: Keyboard interface failed self test\n");
        return -EIO;
    }
    ...
    while (1) {
        ...
        if (status != KBD_REPLY_RESEND) {
            kprintf(KERN_ERR
            "kbd: Keyboard reset failed, no ACK\n");
            return -EIO;
        }
    }
    ...
    if ((status = kbd_wait_for_data()) != KBD_REPLY_POR) {
        kprintf(KERN_ERR
        "kbd: Keyboard reset failed, no POR (%d)\n", status);
        return -EIO;
    }
