...
static int fd_command(unsigned char cmd) {
    ...
    if (!fd_init) kprintf(KERN_WARNING
    "fd: command timeout\n");
    return -ETIMEOUT;
}
...
static int fd_recalibrate(struct fd *fd) {
    ...
    if (wait_for_object(&fd->fdc->done, FD_RECAL_TIMEOUT) < 0) {
        ...
        "fd: timeout waiting for calibrate to complete\n");
        return -ETIMEOUT;
    }
    ...
    while (retries-- > 0) {
        ...
        if (fd->curtrack != track) {
            ...
            if (wait_for_object(&fd->fdc->done, FD_SEEK_TIMEOUT) < 0) {
                kprintf(KERN_WARNING
                "fd: timeout waiting for seek to complete\n");
                fd_recalibrate(fd);
                continue;
            }
            ...
            if ((fd->st0 & 0xE0) != 0x20 || fd->curtrack != track) {
                kprintf(KERN_ERR
                "fd: seek failed, st0 0x%02x, current %d, target %d\n", fd->st0, fd->curtrack, track);
                continue;
            }
        ...
        if (wait_for_object(&fd->fdc->done, FD_XFER_TIMEOUT) < 0) {
            kprintf(KERN_WARNING
            "fd: timeout waiting for transfer to complete\n");
            fd_recalibrate(fd);
            continue;
        }
...
static int fd_read(struct dev *dev, void *buffer, size_t count, blkno_t blkno, int flags) {
    ...
    while (left > 0) {
        ...
        if (result <= 0) {
            kprintf(KERN_ERR
            "fd: error %d reading from floppy\n", result);
            fd_motor_off(fd);
            release_mutex(&fd->fdc->lock);
            return result;
        }
...
void init_fd() {
...
#if 0
    // TODO: the version command times out when no floppy is inserted
    fd_init = 1;
    ...
    for (i = 0; i < DMA_BUFFER_PAGES; i++)
        map_page(fdc.dmabuf + i * PAGESIZE, BTOP(DMA_BUFFER_START) + i, PT_WRITABLE | PT_PRESENT);
    ...
    // TODO: support other geometries: 0=unknown, 1=360K 5 1/4", 2=1.2M 5 1/4", 3=720K 3 1/2", 4=1.44M 3 1/2"
