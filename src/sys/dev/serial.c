    ...
    if (next_serial_portno == 1) {
        for (port = 0; port < 4; port++) {
            ...
            if (iobase != 0) {
                switch (port) {
                    case 0:
                        init_serial_port("com1", iobase, serial_default_irq[0], NULL);
                        break;
                    case 1:
                        init_serial_port("com2", iobase, serial_default_irq[1], NULL);
                        break;
                    case 2:
                        init_serial_port("com3", iobase, serial_default_irq[2], NULL);
                        break;
                    case 3:
                        init_serial_port("com4", iobase, serial_default_irq[3], NULL);
                        break;
                }
            }
        }
