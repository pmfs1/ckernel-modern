...
static struct board board_tbl[] = {
        {"Intel", "Intel i82559 rev 8",                          BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x1229), 0xffffffff, 0, 0, 8, 0xff, HasChksum},
        {"Intel", "Intel PCI EtherExpress Pro100",               BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x1229), 0xffffffff, 0, 0, 0, 0,    0},
        {"Intel", "Intel EtherExpress Pro/100+ i82559ER",        BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x1209), 0xffffffff, 0, 0, 0, 0,    ResetMII},
        {"Intel", "Intel EtherExpress Pro/100 type 1029",        BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x1029), 0xffffffff, 0, 0, 0, 0,    0},
        {"Intel", "Intel EtherExpress Pro/100 type 1030",        BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x1030), 0xffffffff, 0, 0, 0, 0,    0},
        {"Intel", "Intel Pro/100 V Network",                     BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x2449), 0xffffffff, 0, 0, 0, 0,    0},
        {"Intel", "Intel Pro/100 VE (type 1031)",                BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x1031), 0xffffffff, 0, 0, 0, 0,    0},
        {"Intel", "Intel Pro/100 VM (type 1038)",                BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x1038), 0xffffffff, 0, 0, 0, 0,    0},
        {"Intel", "Intel Pro/100 VM (type 1039)",                BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x1039), 0xffffffff, 0, 0, 0, 0,    0},
        {"Intel", "Intel Pro/100 VM (type 103a)",                BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x103a), 0xffffffff, 0, 0, 0, 0,    0},
        {"HP",    "HP/Compaq D510 Intel Pro/100 VM",             BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x103b), 0xffffffff, PCI_UNITCODE(
                0x0e11,
                0x0012),                                                                                           0, 0, 0,    0},
        {"Intel", "Intel Pro/100 VM (type 103b)",                BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x103b), 0xffffffff, 0, 0, 0, 0,    0},
        {"Intel", "Intel Pro/100 VM (unknown type series 1030)", BUSTYPE_PCI, PCI_UNITCODE(0x8086,
                                                                                           0x1030), 0xfffffff0, 0, 0, 0, 0,    0},
        {NULL,},
};
...
__inline static void wait_for_cmd_done(long cmd_ioaddr) {
    ...
    "eepro100: Command %2.2x was not immediately accepted, %d ticks!\n", delayed_cmd, wait);
}
...
static int mdio_read(long ioaddr, int phy_id, int location) {
    ...
    do {
        ...
        if (--boguscnt < 0) {
            kprintf(KERN_WARNING
            "eepro100: mdio_read() timed out with val = %8.8x\n", val);
            break;
        }
...
static int mdio_write(long ioaddr, int phy_id, int location, int value) {
    ...
    do {
        ...
        if (--boguscnt < 0) {
            kprintf(KERN_WARNING
            "eepro100: mdio_write() timed out with val = %8.8x\n", val);
            break;
        }
    } while (!(val & 0x10000000));
...
static void do_slow_command(struct dev *dev, int cmd) {
    ...
    if (wait > 100) kprintf(KERN_WARNING
    "%s: Command %4.4x was never accepted (%d polls)!\n", dev->name, inp(cmd_ioaddr), wait);
    outp(cmd_ioaddr, cmd);
    ...
    kprintf(KERN_WARNING
    "%s: Command %4.4x was not accepted after %d polls!  Current status %8.8x\n", dev->name, cmd, wait, inpd(
            sp->iobase + SCBStatus));
}
...
static int speedo_close(struct dev *dev) {
    ...
    kprintf("%s: Shutting down ethercard, status was %4.4x. Allocation failures: %d\n", dev->name,
            inpw(ioaddr + SCBStatus), sp->alloc_failures);
...
static void speedo_timer(void *arg) {
    ...
    if (sp->cur_tx - sp->dirty_tx > 1 && (get_ticks() - sp->trans_start) > TX_TIMEOUT &&
        (get_ticks() - sp->last_cmd_time) > TX_TIMEOUT) {
        if (status == 0xffff) {
            if (get_ticks() - sp->last_reset > 10 * HZ) {
                sp->last_reset = get_ticks();
                kprintf(KERN_WARNING
                "%s: The EEPro100 chip is missing!\n", dev->name);
            }
            ...
            if (!sp->polling) {
                if (get_ticks() - sp->last_reset > 10 * HZ) {
                    kprintf(KERN_WARNING
                    "%s: IRQ %d is physically blocked! (%4.4x) Failing back to low-rate polling\n", dev->name, sp->irq, status);
                    sp->last_reset = get_ticks();
                }
...
static void speedo_tx_timeout(struct dev *dev) {
    ...
    kprintf(KERN_WARNING
    "%s: Transmit timed out: status %4.4x  %4.4x at %d/%d commands %8.8x %8.8x %8.8x\n",
...
static void speedo_intr_error(struct dev *dev, int intr_status) {
    ...
    if (intr_status & IntrRxSuspend) {
        if ((intr_status & 0x003c) == 0x0028)
            ...
            // No resources (why?!)
            kprintf(KERN_ERR
            "%s: Unknown receiver error, status=%#4.4x\n", dev->name, intr_status);
    ...
    while (sp->rx_ringp[entry] != NULL && (status = sp->rx_ringp[entry]->status) & RxComplete) {
        ...
        if ((status & (RxErrTooBig | RxOK | 0x0f90)) != RxOK) {
            if (status & RxErrTooBig) {
                ...
                sp->stats.rx_errors++;
                kprintf(KERN_WARNING
                "%s: Anomalous event in speedo_rx(), status %8.8x\n", dev->name, status);
            }
        } else {
            struct pbuf *p;
            ...
            while (sp->cur_tx - dirty_tx > 0) {
                ...
                if ((status & StatusComplete) == 0) {
                    ...
                    if (sp->cur_tx - dirty_tx > 2 &&
                        (sp->tx_ring[(dirty_tx + 1) % TX_RING_SIZE].status & StatusComplete)) {
                        kprintf(KERN_ERR
                        "%s: Command unit failed to mark command %8.8x as complete at %d\n", dev->name, status, dirty_tx);
                    } else {
...
int __declspec(dllexport) install(struct unit *unit, char *opts) {
...

    {
        ...
        if (sum != 0xBABA) kprintf(KERN_WARNING
        "eepro100: Invalid EEPROM checksum %#4.4x!\n", sum);
    }
    ...
    if (sp->rx_bug) kprintf(KERN_INFO
    "%s: Receiver lock-up workaround activated\n", dev->name);

    kprintf(KERN_INFO
    "%s: %s%s iobase 0x%x irq %d mac %la\n", dev->name, eeprom[3] & 0x0100 ? "OEM "
                                                                           : "", unit->productname, ioaddr, irq, &sp->hwaddr);
