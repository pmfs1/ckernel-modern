...
static int atapi_packet_read(struct hd *hd, unsigned char *pkt, int pktlen, void *buffer, size_t bufsize) {
    ...
    if (result != 0) {
        ...
        "atapi_packet_read: busy waiting for packet ready (0x%02x)\n", result);
    ...
    while (1) {
        ...
        if (wait_for_object(&hdc->ready, HDTIMEOUT_XFER) < 0) {
            kprintf(KERN_WARNING
            "hd_read: timeout waiting for interrupt\n");
            hdc->result = -EIO;
            break;
        }
        reset_event(&hdc->ready);
...
//kprintf("wait\n");
        ...
        if (wait_for_object(&hdc->ready, HDTIMEOUT_XFER) < 0) {
            kprintf(KERN_ERR
            "hd_write: timeout waiting for interrupt\n");
            hdc->result = -EIO;
            break;
        }
        if (hdc->result < 0) break;
...
static int hd_read_udma(struct dev *dev, void *buffer, size_t count, blkno_t blkno, int flags) {
    ...
    while (sectsleft > 0) {
        ...
        if (wait_for_object(&hdc->ready, HDTIMEOUT_XFER) < 0) {
            kprintf(KERN_WARNING
            "hd: timeout waiting for read to complete\n");
            stop_dma(hdc);
            result = -EIO;
            break;
        }
...
static int hd_write_udma(struct dev *dev, void *buffer, size_t count, blkno_t blkno, int flags) {
    ...
    while (sectsleft > 0) {
        ...
        if (wait_for_object(&hdc->ready, HDTIMEOUT_XFER) < 0) {
            kprintf(KERN_WARNING
            "hd: timeout waiting for write to complete\n");
            stop_dma(hdc);
            result = -EIO;
            break;
        }
...
static int create_partitions(struct hd *hd) {
    ...
    if (rc < 0) {
        kprintf(KERN_ERR
        "%s: error %d reading partition table\n", device(hd->devno)->name, rc);
        return rc;
    }
    ...
    if (mbr->signature != MBR_SIGNATURE) {
        kprintf(KERN_ERR
        "%s: illegal boot sector signature\n", device(hd->devno)->name);
        return -EIO;
    }
    ...
    for (i = 0; i < HD_PARTITIONS; i++) {
        ...
        if (mbr->parttab[i].systid != 0) {
            ...
            if (devno == NODEV) {
                ...
                kprintf(KERN_INFO
                "%s: partition %d on %s, %dMB (type %02x)\n", devname, i, device(hd->devno)->name,
                        mbr->parttab[i].numsect / ((1024 * 1024) / SECTORSIZE), mbr->parttab[i].systid);
            } else {
                dev_close(devno);
            }
        }
    ...
    if (ideprobe) {
        ...
        *slaveif = HDIF_NONE;
        ...
        if (*masterif != HDIF_NONE) {
            ...
            if (rc < 0) {
                kprintf(KERN_ERR
                "hd: error %d waiting for reset to complete on master device\n");
                *masterif = HDIF_NONE;
            }
        }
        ...
        if (*slaveif != HDIF_NONE) {
            ...
            if (rc < 0) {
                kprintf(KERN_ERR
                "hd: error %d waiting for reset to complete on slave device\n");
                *slaveif = HDIF_NONE;
            }
        }
...
static void setup_hd(struct hd *hd, struct hdc *hdc, char *devname, int drvsel, int udmasel, int iftype) {
    ...
    if (hd->multsect > 1) {
        ...
        if (rc < 0) {
            kprintf(KERN_WARNING
            "hd: unable to set multi sector mode\n");
            hd->multsect = 1;
        }
    }
    ...
    if (hd->udmamode != -1) {
        ...
        if (rc < 0) kprintf(KERN_WARNING
        "hd: unable to enable UDMA mode\n");
    }
    ...
    if (hd->media == IDE_DISK) {
        ...
        kprintf(KERN_ERR
        "%s: unknown media type 0x%02x (iftype %d, config 0x%04x)\n", devname, hd->media, hd->iftype, hd->param.config);
        return;
    }

    kprintf(KERN_INFO
    "%s: %s", device(hd->devno)->name, hd->param.model);
...
void init_hd() {
    ...
    if (numhd >= 1) {
        ...
        if (rc < 0) {
            kprintf(KERN_ERR
            "hd: error %d initializing primary IDE controller\n", rc);
        } else {
            if (numhd >= 1 && masterif > HDIF_UNKNOWN)
                setup_hd(&hdtab[0], &hdctab[0], "hd0", HD0_DRVSEL, BM_SR_DRV0, masterif);
            if (numhd >= 2 && slaveif > HDIF_UNKNOWN)
                setup_hd(&hdtab[1], &hdctab[0], "hd1", HD1_DRVSEL, BM_SR_DRV1, slaveif);
        }
    }
    ...
    if (numhd >= 3) {
        ...
        if (rc < 0) {
            kprintf(KERN_ERR
            "hd: error %d initializing secondary IDE controller\n", rc);
        } else {
            if (numhd >= 3 && masterif > HDIF_UNKNOWN)
                setup_hd(&hdtab[2], &hdctab[1], "hd2", HD0_DRVSEL, BM_SR_DRV0, masterif);
            if (numhd >= 4 && slaveif > HDIF_UNKNOWN)
                setup_hd(&hdtab[3], &hdctab[1], "hd3", HD1_DRVSEL, BM_SR_DRV1, slaveif);
        }
    }
}
