...
static int relocate_module(struct module *mod) {
    ...
    if (!reloc) return 0;
    end = (struct image_base_relocation *) ((char *) reloc +
                                            get_image_directory_size(mod->hmod, IMAGE_DIRECTORY_ENTRY_BASERELOC));
...
static int protect_module(struct module *mod) {
    ...
    for (i = 0; i < imghdr->header.number_of_sections; i++) {
        int protect;
        unsigned long scn = imghdr->sections[i].characteristics &
        ...
        mod->db->protect_region(RVA(mod->hmod, imghdr->sections[i].virtual_address),
                                imghdr->sections[i].size_of_raw_data, protect);
    }
...
static int remove_module(struct module *mod) {
    ...
    if (mod->flags & MODULE_INITIALIZED) {
        ...
        if (imghdr->header.characteristics & IMAGE_FILE_DLL) {
            ((int (__stdcall *)(hmodule_t, int, void *)) get_entrypoint(mod->hmod))(mod->hmod, DLL_PROCESS_DETACH,
                                                                                    NULL);
        }
    ...
    while (1) {
        if (get_image_header(m->hmod)->header.characteristics & IMAGE_FILE_DLL) {
            if ((flags & MODLOAD_NOINIT) == 0 || m != mod) {
                ...
                ok = ((int (__stdcall *)(hmodule_t, int, void *)) get_entrypoint(m->hmod))(m->hmod, DLL_PROCESS_ATTACH,
                                                                                           NULL);
...
static struct image_resource_directory_entry *
find_resource(char *resbase, struct image_resource_directory *dir, char *id) {
...
int init_module_database(struct moddb *db, char *name, hmodule_t hmod, char *libpath, struct section *aliassect,
                         int flags) {
    char *basename;
    char *p;
    ...
    db->modules->refcnt = 1;
    db->modules->flags = MODULE_LOADED | MODULE_IMPORTS_REFED | MODULE_RESOLVED | MODULE_RELOCATED | MODULE_BOUND |
                         MODULE_PROTECTED | MODULE_INITIALIZED;
