...
int _output(FILE *stream, const char *format, va_list args) {
    ...
    while (*fmt) {
        ...
        switch (*fmt++) {
            ...
            case '+':
                flags |= FL_SIGN;
                break;  // '+' => Force sign indicator
            case ' ':
                flags |= FL_SIGNSP;
                break;  // ' ' => Force sign or space
            case '#':
                flags |= FL_ALTERNATE;
                break;  // '#' => Alternate form
            case '0':
                flags |= FL_LEADZERO;
                break;  // '0' => Pad with leading zeros
            default :
                fmt--; // No format flag
        }
        ...
        switch (ch) {
            ...
            case_int:
            {
                ...
                unsigned __int64
                number64; // Number to convert (64 bit)
