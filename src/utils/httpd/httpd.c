...
struct httpd_server *httpd_initialize(struct section *cfg) {
    ...
    parse_log_columns(server, getstrconfig(cfg, "logcolumns",
...
struct httpd_context *httpd_add_context(struct httpd_server *server, char *alias, httpd_handler handler, void *userdata,
                                        struct section *cfg) {
...
httpd_add_file_context(struct httpd_server *server, char *alias, char *location, struct section *cfg) {
    struct httpd_context *context;
...
struct httpd_context *
httpd_add_resource_context(struct httpd_server *server, char *alias, hmodule_t hmod, struct section *cfg) {
    struct httpd_context *context;
    char modfn[MAXPATH];
    struct stat statbuf;
