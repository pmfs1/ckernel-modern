...
static int vmdk_open(struct vmdkdev *s, const char *filename)
{
    ...
    // Read header
    if (read(hdev, &magic, sizeof(magic)) != sizeof(magic))
        goto fail;
    if (magic != VMDK4_MAGIC)
        goto fail;

    if (read(hdev, &header, sizeof(header)) != sizeof(header))
        goto fail;
    ...
    if (lseek64(hdev, s->l1_table_offset, SEEK_SET) < 0)
        goto fail;
    if (read(hdev, s->l1_table, l1_size) < 0)
        goto fail;
    ...
    if (s->l1_backup_table_offset)
    {
        ...
        if (lseek64(hdev, s->l1_backup_table_offset, SEEK_SET) < 0)
            goto fail;
        if (read(hdev, s->l1_backup_table, l1_size) < 0)
            goto fail;
    }
...
static uint64_t get_cluster_offset(struct vmdkdev *s, uint64_t offset, int allocate)
{
    ...
    if (read(s->hdev, l2_table, s->l2_size * sizeof(uint32_t)) != s->l2_size * sizeof(uint32_t))
        return 0;
...
found:
    ...
    if (!cluster_offset)
    {
        ...
        lseek64(s->hdev, pos, SEEK_SET);
        if (write(s->hdev, &tmp, sizeof(tmp)) != sizeof(tmp))
            return 0;
...
static int vmdk_write(struct vmdkdev *s, int64_t sector_num, const uint8_t *buf, int nb_sectors)
{
    ...
    while (nb_sectors > 0)
    {
        ...
        lseek64(s->hdev, pos, SEEK_SET);
        if (write(s->hdev, buf, n * 512) != n * 512)
            return -1;
        nb_sectors -= n;
        sector_num += n;
        buf += n * 512;
...
static int vmdk_create(const char *filename, int64_t total_size, int flags)
{
    ...
    // Write all the data
    if (write(hdev, &magic, sizeof(magic)) < 0)
        return -1;
    if (write(hdev, &header, sizeof(header)) < 0)
        return -1;

    pos = header.grain_offset << 9;
    if (ftruncate64(hdev, pos) < 0)
        return -1;

    // Write grain directory
    pos = header.rgd_offset << 9;
    if (lseek64(hdev, pos, SEEK_SET) < 0)
        return -1;
    for (i = 0, tmp = (uint32_t)header.rgd_offset + gd_size; i < gt_count; i++, tmp += gt_size)
    {
        if (write(hdev, &tmp, sizeof(tmp)) < 0)
            return -1;
    }
    ...
    if (lseek64(hdev, pos, SEEK_SET) < 0)
        return -1;
    if (write(hdev, desc, strlen(desc)) < 0)
        return -1;
