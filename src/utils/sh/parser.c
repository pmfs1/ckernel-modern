...
static void parse_error(struct parser *p, char *msg)
{
    ...
    if (p->source)
        fprintf(stderr, ", line %d", p->source->lineno);
    fprintf(stderr, "\n");
    p->errors++;
}
...
static int parse_param(struct parser *p)
{
    ...
    {
    case '#':
        node->nargparam.flags |= S_ARGC;
        break;
    case '*':
        node->nargparam.flags |= S_ARGV;
        break;
    case '@':
        node->nargparam.flags |= S_ARGVS;
        break;
    case '?':
        node->nargparam.flags |= S_EXITCODE;
        break;
    case '-':
        node->nargparam.flags |= S_FLAGS;
        break;
    case '!':
        node->nargparam.flags |= S_BGEXCODE;
        break;
    case '$':
        node->nargparam.flags |= S_PID;
        break;
    }
    ...
    if (node->nargparam.flags & S_ARG)
        node->nargparam.num = atoi(node->nargparam.name);

    // Skip whitespace if inside braces (unusual), otherwise return
    if (!braces)
        return 0;
    while (p->ch > 0 && is_space(p->ch))
        next(p);
    ...
    else
    {
        ...
        {
        case '-':
            node->nargparam.flags |= S_DEFAULT;
            next(p);
            break;
        case '=':
            node->nargparam.flags |= S_ASGNDEF;
            next(p);
            break;
        case '?':
            node->nargparam.flags |= S_ERRNULL;
            next(p);
            break;
        case '+':
            node->nargparam.flags |= S_ALTERNAT;
            next(p);
            break;
        }
    }
...
static int parse_bquoted(struct parser *p)
{
    ...
    if ((cmdlist = parse_compound_list(p)) == NULL)
        return -1;

    // MUST be terminated with right parenthesis or backquote
    if (!parse_expect(p, P_DEFAULT, (bq ? T_BQ : T_RP)))
        return -1;
...
static int parse_unquoted(struct parser *p)
{
    ...
    for (;;)
    {
        ...
        else if (p->ch == '<' || p->ch == '>')
        {
            // Handle redirections
            int fd = (p->ch == '<' ? 0 : 1);
            if (ststrlen(p->mark) > 0)
                fd = atoi(ststr(p->mark));
            if (fd != -1)
                return parse_redir(p, (p->ch == '<' ? R_IN : R_OUT), fd);
        }
        ...
        else if (is_esc(p->ch))
        {
            ...
            if (p->tok != T_ASSIGN && !(p->flags & P_NOGLOB))
                flags |= S_GLOB;
        }
        ...
        if (p->tok == T_NAME && ststrlen(p->mark) > 0 && p->ch == '=')
            p->tok = T_ASSIGN;
        if (p->tok == T_NAME && !is_name(p->ch))
            p->tok = T_WORD;
...
static union node *parse_grouping(struct parser *p)
{
    ...
    grouping = NULL;
    if (!(tok = parse_expect(p, P_DEFAULT, T_BEGIN | T_LP)))
        return NULL;
    ...
    if (!parse_expect(p, P_DEFAULT, tok == T_BEGIN ? T_END : T_RP))
        return NULL;
...
union node *parse_function(struct parser *p, char *name)
{
    ...
    // Next tokens must be )
    if (!parse_expect(p, P_DEFAULT, T_RP))
        return NULL;

    // Parse the function body
    if ((node->nfunc.cmds = parse_grouping(p)) == NULL)
        return NULL;
...
union node *parse_simple_command(struct parser *p)
{
    ...
    while (!end)
    {
        ...
        case T_LP:
            ...
            if (funcname)
                return parse_function(p, funcname);
...
static union node *parse_if(struct parser *p)
{
    ...
    do
    {
        ...
        *nptr = alloc_node(p, N_IF);
        if ((list = parse_compound_list(p)) == NULL)
            return NULL;
        (*nptr)->nif.test = list;

        // Next token must be a T_THEN
        if (!parse_expect(p, P_DEFAULT, T_THEN))
            return NULL;
    ...
    if (!parse_expect(p, P_DEFAULT, T_FI))
        return NULL;
...
static union node *parse_loop(struct parser *p)
{
    ...
    if (!parse_expect(p, P_DEFAULT, T_DO))
        return NULL;
    ...
    if (!parse_expect(p, P_DEFAULT, T_DONE))
        return NULL;
...
union node *parse_for(struct parser *p)
{
    ...
    if (!parse_expect(p, P_DEFAULT, T_NAME))
        return NULL;
    ...
    if (!parse_expect(p, P_SKIPNL, T_DO))
        return NULL;
...
static union node *parse_case(struct parser *p)
{
    ...
    if (!parse_expect(p, P_DEFAULT, T_WORD | T_NAME | T_ASSIGN))
        return NULL;
    word = parse_getarg(p);

    // Then the keyword 'in' must follow
    if (!parse_expect(p, P_SKIPNL, T_IN))
        return NULL;
    ...
    while (!(parse_gettok(p, P_SKIPNL | P_NOGLOB) & T_ESAC))
    {
        ...
        *cptr = alloc_node(p, N_CASENODE);
        ...
        p->pushback++;
        if (!parse_expect(p, P_DEFAULT, T_RP | T_PIPE))
            return NULL;
        ...
        // Expect esac or ;;
        if (!parse_expect(p, P_DEFAULT, T_ESAC | T_ECASE))
            return NULL;
