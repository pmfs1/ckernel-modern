...
static void print_redir(union node *rdir, FILE *out, int level)
{
    ...
    if (rdir)
    {
        indent(out, level);
        fprintf(out, " redir:\n");
        for (n = rdir; n; n = n->list.next)
            print_node(n, out, level + 1);
    }
}
...
void print_expr(struct expr *expr, FILE *out, int level)
{
    if (expr)
    {
        ...
        if (expr->num != 0)
            fprintf(out, " num=%d", expr->num);
        if (expr->var)
            fprintf(out, " var=%s", expr->var);
        fprintf(out, "\n");
        if (expr->left)
            print_expr(expr->left, out, level + 1);
        if (expr->right)
            print_expr(expr->right, out, level + 1);
    }
}
...
void print_node(union node *node, FILE *out, int level)
{
    ...
    case N_ARGCMD:
        ...
        if (node->nargcmd.list)
            print_list(node->nargcmd.list, out, level + 1);
        break;
