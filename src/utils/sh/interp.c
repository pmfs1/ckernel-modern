    ...
    else
    {
        ...
        case S_ASGNDEF: // ${parameter:=word}
            if (!value)
            {
                ...
                if (job->parent)
                    set_var(job->parent, param->name, value);
            }
            break;
...
static int expand_args(struct stkmark *mark, struct job *job, struct args *args, union node *node)
{
    ...
    case N_ARG:
        if (args)
        {
            ...
            if (args->num == before || ststrlen(mark) > 0)
                add_arg(args, ststr(mark));
        }
        else
        {
    ...
    case N_ARGSTR:
        ...
        {
            if (expand_glob(mark, args, node->nargstr.text) < 0)
                return 1;
        }
        else
        {
...
static int interp_for(struct job *parent, union node *node)
{
    ...
    for (arg = job->args.first; arg; arg = arg->next)
    {
        ...
        for (n = node->nfor.cmds; n; n = n->list.next)
        {
            ...
            if (rc != 0)
                break;
            if (job->flags & (J_BREAK | J_CONTINUE))
                break;
        }
        if (job->flags & J_BREAK)
            break;
...
int interp(struct job *parent, union node *node)
{
    ...
    {
    case N_SIMPLECMD:
        return interp_simple_command(parent, node);
    case N_PIPELINE:
        return interp_pipeline(parent, node);
    case N_CMDLIST:
        return interp_cmdlist(parent, node, 0);
    case N_SUBSHELL:
        return interp_cmdlist(parent, node, 1);
    case N_AND:
        return interp_and(parent, node);
    case N_OR:
        return interp_or(parent, node);
    case N_NOT:
        return interp_not(parent, node);
    case N_IF:
        return interp_if(parent, node);
    case N_FOR:
        return interp_for(parent, node);
    case N_WHILE:
        return interp_loop(parent, node, 0);
    case N_UNTIL:
        return interp_loop(parent, node, 1);
    case N_CASE:
        return interp_case(parent, node);
    case N_FUNCTION:
        return interp_function(parent, node);
    default:
