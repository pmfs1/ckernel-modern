...
int cmd_chown(int argc, char *argv[])
{
    ...
    if (chown(path, pwd ? pwd->pw_uid : -1, grp ? grp->gr_gid : -1) < 0)
        perror(path);
...
int cmd_ifconfig(int argc, char *argv[])
{
    ...
    for (i = 0; i < n / (int)sizeof(struct ifcfg); i++)
    {
        ...
        if (iflist[i].flags & IFCFG_UP)
            printf(" UP");
        if (iflist[i].flags & IFCFG_DHCP)
            printf(" DHCP");
        if (iflist[i].flags & IFCFG_DEFAULT)
            printf(" DEFAULT");
...
int cmd_ps(int argc, char *argv[])
{
    ...
    while (proc)
    {
        printf("%4d %6d %-13s%08x%8d%4d%4d%4d %s\n", proc->id, proc->parent ? proc->parent->id : 0, proc->ident,
               proc->hmod, proc->threadcnt, proc->iob[0], proc->iob[1], proc->iob[2],
    ...
    while (_readdir(dir, &dirp, 1) > 0)
    {
        ...
        else
        {
            ...
            if (verbose)
            {
                ...
                printf("%02d/%02d/%04d %02d:%02d:%02d ", tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900, tm.tm_hour,
                       tm.tm_min, tm.tm_sec);
            }
            else
            {
...
int cmd_sysinfo(int argc, char *argv[])
{
    ...
    printf("mem phys total: %d K avail: %d K virt total: %d K avail: %d K pagesize: %d\n",
           mem.physmem_total / 1024, mem.physmem_avail / 1024, mem.virtmem_total / 1024, mem.virtmem_avail / 1024,
           mem.pagesize);
...
static int exec_builtin(int argc, char *argv[], int *found)
{
    ...
    for (command = cmdtab; command->cmdname; command++)
    {
        if (strcmp(argv[0], command->cmdname) == 0)
            break;
    }
...
int execute_script(char *cmdfile)
{
    ...
    while (fgets(cmdline, sizeof cmdline, f))
    {
        if (*cmdline == ';' || *cmdline == '#')
            continue;
        if (echo)
            fwrite(cmdline, strlen(cmdline), 1, stdout);
...
{
    if (gettib()->proc->term->type == TERM_VT100)
