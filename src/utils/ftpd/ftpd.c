...
int opendata(struct ftpstate *fs) {
    ...
    if (fs->passive) {
        ...
        if (connect(fs->datasock, (struct sockaddr *) &sin, sizeof(sin)) < 0) {
            addreply(fs, 425, "Could not open data connection to %s port %d: %s", inet_ntoa(sin.sin_addr), fs->dataport,
                     strerror(errno));
            close(fs->datasock);
            fs->datasock = -1;
            return -1;
...
void domdtm(struct ftpstate *fs, char *name) {
    ...
    if (stat(filename, &st) < 0) {
        ...
        if (!t) {
            ...
            addreply(fs, 213, "%04d%02d%02d%02d%02d%02d", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour,
                     t->tm_min, t->tm_sec);
        }
    }
}
...
void doport(struct ftpstate *fs, unsigned int ip, unsigned int port) {
    ...
    if (fs->debug)
        addreply(fs, 0, "My data connection endpoint is %s:%d", inet_ntoa(sin.sin_addr), ntohs(sin.sin_port));
...
void dopasv(struct ftpstate *fs, int useepsv) {
    ...
    if (useepsv) {
        ...
        addreply(fs, 227, "Passive mode OK (%d,%d,%d,%d,%d,%d)", (a >> 24) & 255, (a >> 16) & 255, (a >> 8) & 255,
                 a & 255, (p >> 8) & 255, p & 255);
    }
