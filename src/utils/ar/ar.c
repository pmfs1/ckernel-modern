...
int main(int argc, char *argv[])
{
    ...
    while (optind < argc)
    {
        ...
        if (!normalized_input)
        {
            fprintf(stderr, "Memory allocation error processing %s\n", input_file);
            free_archive(&ar);
            return 1;
        }

        fd = open(normalized_input, O_RDONLY | O_BINARY);
        if (fd < 0)
        {
            perror(input_file);
