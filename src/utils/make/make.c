    ...
    while (!feof(f))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (!rule)
                    rule = add_rule(prj, buffer_dup(name));
...
{
    if (prj->build_last)
        prj->build_last->build_next = rule;
    if (!prj->build_first)
        prj->build_first = rule;
    prj->build_last = rule;
}
...
int check_rule(struct project *prj, struct rule *rule)
{
    ...
    {
        if (prj->debug)
            printf("build %s because it does not exist\n", rule->target);
        dirty = 1;
    }
    ...
    while (item)
    {
        ...
        if (dep)
        {
            ...
            {
                if (prj->debug)
                    printf("build %s because %s needs to be built\n", rule->target, item->value);
                dirty = 1;
            }
            ...
            {
                if (prj->debug)
                    printf("build %s because it is older than %s\n", rule->target, item->value);
                dirty = 1;
            }
        }
        else
    ...
    if (prj->phony)
    {
        ...
        while (item)
        {
            ...
            {
                if (prj->debug)
                    printf("build %s because it is a phony target\n", rule->target);
                dirty = 1;
                break;
            }
            item = item->next;
...
int build_targets(struct project *prj)
{
    ...
    while (r)
    {
        ...
        while (command)
        {
            ...
            if (expand_macros(command->value, prj, r, cmd) < 0)
                return -1;
            if (!prj->silent)
                printf("%s\n", cmd->start);
            if (!prj->dry_run)
            {
                if (system(cmd->start) != 0)
                    return -1;
            }
