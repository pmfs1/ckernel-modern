...
sockapi int __stdcall WSAGetLastError()
{
    ...
    if (err == 0)
        return 0;
    if (err == EAGAIN)
        return EWOULDBLOCK + WSAERRBASE;
    if (err == EBADF)
        return ENOTSOCK + WSAERRBASE;
    if (err < 45)
        return err + WSAERRBASE;
    if (err < 82)
        return (err - 10) + WSAERRBASE;
    if (err == EHOSTNOTFOUND)
        return 1001 + WSAERRBASE;
    if (err == ETRYAGAIN)
        return 1002 + WSAERRBASE;
    if (err == ENORECOVERY)
        return 1003 + WSAERRBASE;
    if (err == ENODATA)
        return 1004 + WSAERRBASE;
    ...
    LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine)
{
    ...
    if (lpOverlapped != NULL)
        panic("Overlapped I/O not implemented in WSARecvFrom");
    if (lpCompletionRoutine != NULL)
        panic("Completion routines not implemented in WSARecvFrom");
    ...
    LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine)
{
    ...
    if (lpOverlapped != NULL)
        panic("Overlapped I/O not implemented in WSASendTo");
    if (lpCompletionRoutine != NULL)
        panic("Completion routines not implemented in WSASendTo");
    ...
    LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine)
{
    ...
    if (dwIoControlCode == SIO_GET_INTERFACE_LIST)
    {
        ...
        for (i = 0; i < numifs; i++)
        {
            ...
            if (iflist[i].flags & IFCFG_UP)
                ifinfo[i].iiFlags |= IFF_UP;
            if (iflist[i].flags & IFCFG_LOOPBACK)
                ifinfo[i].iiFlags |= IFF_LOOPBACK;
        ...
        if (lpcbBytesReturned)
            *lpcbBytesReturned = numifs * sizeof(INTERFACE_INFO);
        free(iflist);
        return 0;
    }
    else
    ...
    long lNetworkEvents)
{
    ...
    if (hEventObject == 0 && lNetworkEvents == 0)
        return 0;
    ...
    LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine)
{
    ...
    if (lpOverlapped != NULL)
        panic("Overlapped I/O not implemented in WSARecv");
    if (lpCompletionRoutine != NULL)
        panic("Completion routines not implemented in WSARecv");
    ...
    LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine)
{
    ...
    if (lpOverlapped != NULL)
        panic("Overlapped I/O not implemented in WSASend");
    if (lpCompletionRoutine != NULL)
        panic("Completion routines not implemented in WSASend");
