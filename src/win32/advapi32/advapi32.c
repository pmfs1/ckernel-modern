...
TRACE("RegCloseKey");
syslog(LOG_DEBUG,
"warning: RegCloseKey not implemented, ignored");
return -1;
}
...
syslog(LOG_DEBUG,
"warning: RegCreateKeyExA(%p,%s) ignored", hKey, lpSubKey);
return -1;
}
...
syslog(LOG_DEBUG,
"warning: RegDeleteValueA(%p,%s) ignored", hKey, lpValueName);
return -1;
}
...
syslog(LOG_DEBUG,
"warning: RegSetValueExA(%p,%s) ignored", hKey, lpValueName);
return -1;
}
...
TRACE("RegDeleteKeyA");
syslog(LOG_DEBUG,
"warning: RegDeleteKeyA(%p,%s) ignored", hKey, lpSubKey);
return -1;
}
...
syslog(LOG_DEBUG,
"warning: RegQueryInfoKeyA ignored");
return -1;
}
...
syslog(LOG_DEBUG,
"warning: RegQueryValueExA ignored");
return -1;
}
...
syslog(LOG_DEBUG,
"warning: RegQueryValueExW ignored");
return -1;
}
...
if (wNumStrings > 0) {
syslog(level,
"%s: %s", hEventLog, lpStrings[0]);
} else {
syslog(level,
"%s: message #%d", hEventLog, dwEventID);
}
...
StartServiceCtrlDispatcherA(
        CONST
LPSERVICE_TABLE_ENTRY lpServiceTable
) {
