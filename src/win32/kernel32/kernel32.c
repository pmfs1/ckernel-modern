...
void raise_exception(EXCEPTION_RECORD *rec, CONTEXT *ctxt)
{
        ...
        while (frame != (PEXCEPTION_FRAME)0xFFFFFFFF)
        {
                ...
                case ExceptionNestedException:
                        if (nested_frame < dispatch)
                                nested_frame = dispatch;
                        rec->ExceptionFlags |= EH_NESTED_CALL;
                        break;
    ...
    LPCTSTR lpName)
{
        TRACE("CreateEventA");
        if (lpName != NULL)
                panic("named event not supported");
        return (HANDLE)
    ...
    LPCTSTR lpName)
{
        ...
        if (lpName != NULL)
                panic("named semaphores not supported");
        return (HANDLE)
            mksem(lInitialCount);
}
    ...
    LPWIN32_FIND_DATAW lpFindFileData)
{
        ...
        convert_filename_to_unicode(fd
                                        .cFileName,
                                    lpFindFileData->cFileName, MAXPATH);
        return fh;
}
    ...
    LPTSTR lpBuffer)
{
        ...
        if (!getcwd(curdir, MAXPATH))
                return 0;
        if (nBufferLength <
            strlen(curdir) + 3)
                return strlen(curdir) + 3;
        lpBuffer[0] = 'c';
        lpBuffer[1] = ':';
    ...
    LPDWORD lpFileSizeHigh)
{
        ...
        size.QuadPart = fstat64((handle_t)
                                    hFile,
                                NULL);
        if (size.QuadPart < 0)
        ...
        if (lpCreationTime)
        {
                *(
                    unsigned __int64
                        *)
                    lpCreationTime =
                    (unsigned __int64)statbuf.st_ctime * SECTIMESCALE + EPOC;
        }
        ...
        if (lpLastAccessTime)
        {
                *(
                    unsigned __int64
                        *)
                    lpLastAccessTime =
                    (unsigned __int64)statbuf.st_atime * SECTIMESCALE + EPOC;
        }
        ...
        if (lpLastWriteTime)
        {
                *(
                    unsigned __int64
                        *)
                    lpLastWriteTime =
                    (unsigned __int64)statbuf.st_mtime * SECTIMESCALE + EPOC;
        }
    ...
    LPCTSTR lpNewFileName)
{
        TRACE("MoveFileA");
        if (
            rename(lpExistingFileName, lpNewFileName) < 0)
                return FALSE;
        ...
        lpOverlapped)
{
        ...
        TRACE("ReadFile");
        if (lpOverlapped)
                panic("Overlapped I/O not implemented in ReadFile");
    ...
    DWORD dwMoveMethod)
{
        ...
        if (lpDistanceToMoveHigh)
        {
                ...
                retval.QuadPart = lseek64((handle_t)
                                              hFile,
                                          offset.QuadPart, dwMoveMethod);
        ...
        lpFileTime)
{
        ...
        *(
            ...
            ((
                 ...
                 1000000 +
             (unsigned __int64)(lpSystemTime->wMilliseconds) * 1000) *
                MICROTIMESCALE +
            EPOC;
        return TRUE;
}
        ...
        lpOverlapped)
{
        ...
        TRACE("WriteFile");
        if (lpOverlapped)
                panic("Overlapped I/O not implemented in WriteFile");
