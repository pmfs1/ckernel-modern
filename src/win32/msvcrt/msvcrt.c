...
void __getmainargs(int *pargc, char ***pargv, char ***penvp, int dowildcard, _startupinfo *startinfo)
{
    ...
    *pargv = malloc(sizeof(char *) * *pargc);
    if (*pargv)
        parse_args(gettib()->proc->cmdline, *pargv);
...
unsigned long
_beginthreadex(void *security, unsigned stack_size, unsigned(__stdcall *start_address)(void *), void *arglist,
...
{
    int ret = 0;
    while (!(ret = *(unsigned short *)s1 - *(unsigned short *)s2) && *s2)
        ++s1, ++s2;
